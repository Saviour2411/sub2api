...
func TestNormalizeOpenAIResponsesCompactRequest_NonRemoteV2BodySignalPromoted(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			reqStream, streamOK := parseOpenAICompatibleStream(normalized)
			require.True(t, streamOK)
			require.False(t, reqStream, "提升后的 compact 上游请求必须保持非流式")
			...
			if tt.wantMarked {
				...
				clientStream, clientStreamOK := parseOpenAIResponsesClientStream(c, normalized)
				require.True(t, clientStreamOK)
				require.True(t, clientStream, "归一化删除 stream 后仍应保留下游 SSE 语义")
			}
		})
	}
}
