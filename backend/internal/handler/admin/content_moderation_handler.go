...
type contentModerationConfigRequest struct {
	...
	// ProxyID 控制审核请求代理：nil 不修改，0 清除（直连），大于 0 指定代理。
...
func (h *ContentModerationHandler) UpdateConfig(c *gin.Context) {
	...
	cfg, err := h.service.UpdateConfig(c.Request.Context(), service.UpdateContentModerationConfigInput{
		...
		LocalAuditEnabled:               req.LocalAuditEnabled,
		LocalAuditMaxStorageGB:          req.LocalAuditMaxStorageGB,
		LocalAuditScenePolicy:           req.LocalAuditScenePolicy,
		LocalAuditExcludeImage:          req.LocalAuditExcludeImage,
		LocalAuditClientPatterns:        req.LocalAuditClientPatterns,
		LocalAuditToolPatterns:          req.LocalAuditToolPatterns,
		LocalAuditMaxCaptureConcurrency: req.LocalAuditMaxCaptureConcurrency,
	})
	if err != nil {
		response.ErrorFrom(c, err)
		return
...
func (h *ContentModerationHandler) ListLocalAuditRecords(c *gin.Context) {
	page, pageSize := response.ParsePagination(c)
	filter := service.ContentModerationLocalAuditListFilter{
		Pagination: pagination.PaginationParams{
			Page:      page,
			PageSize:  pageSize,
			SortOrder: pagination.SortOrderDesc,
	...
	items, pageResult, err := h.service.ListLocalAuditRecords(c.Request.Context(), filter)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *ContentModerationHandler) GetLocalAuditRecord(c *gin.Context) {
	record, err := h.service.GetLocalAuditRecord(c.Request.Context(), c.Param("id"))
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *ContentModerationHandler) DownloadLocalAuditRecord(c *gin.Context) {
	raw, filename, err := h.service.ReadLocalAuditRecordBytes(c.Request.Context(), c.Param("id"))
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *ContentModerationHandler) DeleteLocalAuditRecord(c *gin.Context) {
	if err := h.service.DeleteLocalAuditRecord(c.Request.Context(), c.Param("id")); err != nil {
		response.ErrorFrom(c, err)
		return
	}
