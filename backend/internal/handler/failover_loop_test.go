...
func TestHandleFailoverError_BasicSwitch(t *testing.T) {
	t.Run("显式停止不切换账号且旧错误默认仍切换", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, 100, service.PlatformGrok, stopErr, maxSameAccountRetries)
		...
		action = fs.HandleFailoverError(context.Background(), mock, 100, service.PlatformGrok, legacyErr, maxSameAccountRetries)
	...
	t.Run("已取消的认证失败不改变切换状态", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(ctx, mock, 101, service.PlatformGrok, err, maxSameAccountRetries)
	...
	t.Run("非重试错误_Antigravity_第一次切换无延迟", func(t *testing.T) {
		...
		start := time.Now()
		action := fs.HandleFailoverError(context.Background(), mock, 100, service.PlatformAntigravity, err, maxSameAccountRetries)
		elapsed := time.Since(start)
	...
	t.Run("非重试错误_Antigravity_第二次切换有1秒延迟", func(t *testing.T) {
		...
		err := newTestFailoverErr(500, false, false)
		start := time.Now()
		action := fs.HandleFailoverError(context.Background(), mock, 200, service.PlatformAntigravity, err, maxSameAccountRetries)
		elapsed := time.Since(start)
	...
	t.Run("连续切换直到耗尽", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, 100, "openai", err1, maxSameAccountRetries)
		require.Equal(t, FailoverContinue, action)
		require.Equal(t, 1, fs.SwitchCount)

		// 第二次切换：1→2
		err2 := newTestFailoverErr(502, false, false)
		action = fs.HandleFailoverError(context.Background(), mock, 200, "openai", err2, maxSameAccountRetries)
		require.Equal(t, FailoverContinue, action)
		require.Equal(t, 2, fs.SwitchCount)

		// 第三次已耗尽：SwitchCount(2) >= MaxSwitches(2)
		err3 := newTestFailoverErr(503, false, false)
		action = fs.HandleFailoverError(context.Background(), mock, 300, "openai", err3, maxSameAccountRetries)
		require.Equal(t, FailoverExhausted, action)
...
func TestHandleFailoverError_CacheBilling(t *testing.T) {
	t.Run("hasBoundSession为true且实际切换时设置ForceCacheBilling", func(t *testing.T) {
		...
		fs.HandleFailoverError(context.Background(), mock, 100, "openai", err, maxSameAccountRetries)
	...
	t.Run("一旦设置不会被后续错误重置", func(t *testing.T) {
		...
		fs.HandleFailoverError(context.Background(), mock, 100, "openai", err1, maxSameAccountRetries)
		require.True(t, fs.ForceCacheBilling)

		// 第二次：ForceCacheBilling=false → 仍然保持 true
		err2 := newTestFailoverErr(502, false, false)
		fs.HandleFailoverError(context.Background(), mock, 200, "openai", err2, maxSameAccountRetries)
...
func TestHandleFailoverError_SameAccountRetry(t *testing.T) {
	...
	t.Run("不同账号独立跟踪重试次数", func(t *testing.T) {
		...
		// 账号 200 第一次重试（独立计数）
		action = fs.HandleFailoverError(context.Background(), mock, 200, "openai", err, maxSameAccountRetries)
		require.Equal(t, FailoverContinue, action)
...
func TestHandleFailoverError_UsesAccountRetryLimit(t *testing.T) {
	mock := &mockTempUnscheduler{}
	fs := NewFailoverState(3, false)
...
func TestHandleFailoverError_StrictFailureSchedulingSkipsSameAccountRetry(t *testing.T) {
	...
	action := fs.HandleFailoverError(context.Background(), mock, 42, "openai", err, maxSameAccountRetries)
...
func TestHandleFailoverError_TempUnschedule(t *testing.T) {
	...
	t.Run("重试错误耗尽后调用TempUnschedule_传入正确参数", func(t *testing.T) {
		...
		for i := 0; i < maxSameAccountRetries; i++ {
			fs.HandleFailoverError(context.Background(), mock, 42, "openai", err, maxSameAccountRetries)
		}
...
func TestHandleFailoverError_ContextCanceled(t *testing.T) {
	t.Run("同账号重试sleep期间context取消", func(t *testing.T) {
		...
		start := time.Now()
		action := fs.HandleFailoverError(ctx, mock, 100, "openai", err, maxSameAccountRetries)
		elapsed := time.Since(start)
	...
	t.Run("Antigravity延迟期间context取消", func(t *testing.T) {
		...
		start := time.Now()
		action := fs.HandleFailoverError(ctx, mock, 100, service.PlatformAntigravity, err, maxSameAccountRetries)
		elapsed := time.Since(start)
...
func TestHandleFailoverError_FailedAccountIDs(t *testing.T) {
	t.Run("切换时添加到失败列表", func(t *testing.T) {
		...
		fs.HandleFailoverError(context.Background(), mock, 100, "openai", newTestFailoverErr(500, false, false), maxSameAccountRetries)
		require.Contains(t, fs.FailedAccountIDs, int64(100))

		fs.HandleFailoverError(context.Background(), mock, 200, "openai", newTestFailoverErr(502, false, false), maxSameAccountRetries)
		require.Contains(t, fs.FailedAccountIDs, int64(200))
	...
	t.Run("耗尽时也添加到失败列表", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, 100, "openai", newTestFailoverErr(500, false, false), maxSameAccountRetries)
		require.Equal(t, FailoverExhausted, action)
		require.Contains(t, fs.FailedAccountIDs, int64(100))
	})
	...
	t.Run("同账号重试期间不添加到失败列表", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, 100, "openai", newTestFailoverErr(400, true, false), maxSameAccountRetries)
		require.Equal(t, FailoverContinue, action)
		require.NotContains(t, fs.FailedAccountIDs, int64(100))
	})
...
func TestHandleFailoverError_IntegrationScenario(t *testing.T) {
	t.Run("模拟完整failover流程_多账号混合重试与切换", func(t *testing.T) {
		...
		for i := 0; i < maxSameAccountRetries; i++ {
			action := fs.HandleFailoverError(context.Background(), mock, 100, "openai", retryErr, maxSameAccountRetries)
			require.Equal(t, FailoverContinue, action)
		...
		action = fs.HandleFailoverError(context.Background(), mock, 200, "openai", switchErr, maxSameAccountRetries)
		require.Equal(t, FailoverContinue, action)
		require.Equal(t, 2, fs.SwitchCount)

		// 4. 账号 300 遇到不可重试错误 → 再切换
		action = fs.HandleFailoverError(context.Background(), mock, 300, "openai", switchErr, maxSameAccountRetries)
		require.Equal(t, FailoverContinue, action)
		require.Equal(t, 3, fs.SwitchCount)

		// 5. 账号 400 → 已耗尽 (SwitchCount=3 >= MaxSwitches=3)
		action = fs.HandleFailoverError(context.Background(), mock, 400, "openai", switchErr, maxSameAccountRetries)
		require.Equal(t, FailoverExhausted, action)
	...
	t.Run("模拟Antigravity平台完整流程", func(t *testing.T) {
		...
		start = time.Now()
		action = fs.HandleFailoverError(context.Background(), mock, 300, service.PlatformAntigravity, err, maxSameAccountRetries)
		elapsed = time.Since(start)
		require.Equal(t, FailoverExhausted, action)
	...
	t.Run("ForceCacheBilling通过错误标志设置", func(t *testing.T) {
		...
		fs.HandleFailoverError(context.Background(), mock, 300, "openai", err3, maxSameAccountRetries)
...
func TestHandleFailoverError_EdgeCases(t *testing.T) {
	...
	t.Run("AccountID为0也能正常跟踪", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, 0, "openai", err, maxSameAccountRetries)
		require.Equal(t, FailoverContinue, action)
	...
	t.Run("负AccountID也能正常跟踪", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, -1, "openai", err, maxSameAccountRetries)
		require.Equal(t, FailoverContinue, action)
	...
	t.Run("空平台名称不触发Antigravity延迟", func(t *testing.T) {
		...
		start := time.Now()
		action := fs.HandleFailoverError(context.Background(), mock, 100, "", err, maxSameAccountRetries)
		elapsed := time.Since(start)
