...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	...
	return &PublicSettings{
		...
		CanvasEnabled:            settings[SettingKeyCanvasEnabled] != "false",
...
func (s *SettingService) GetModelMarketplaceRuntime(ctx context.Context) ModelMarketplaceRuntime {
	runtime := ModelMarketplaceRuntime{Enabled: true}
	if s == nil || s.settingRepo == nil {
		return runtime
	}
	vals, err := s.settingRepo.GetMultiple(ctx, []string{
	...
	if err != nil {
		slog.Warn("failed to get model marketplace settings", "error", err)
		return runtime
	}
	runtime.Enabled = vals[SettingKeyModelMarketplaceEnabled] != "false"
	runtime.Intro = strings.TrimSpace(vals[SettingKeyModelMarketplaceIntro])
	runtime.GroupIDs = normalizeModelMarketplaceGroupIDs(vals[SettingKeyModelMarketplaceGroupIDs])
	return runtime
}

func normalizeModelMarketplaceGroupIDs(raw string) []int64 {
	raw = strings.TrimSpace(raw)
	if raw == "" {
		return nil
	}
...
type PublicSettingsInjectionPayload struct {
	...
	// ChannelMonitorHideThroughput 对外公开，使用户界面无需只依赖 API 脱敏
	// 就能隐藏 RPM/TPM，形成纵深防护。
	ChannelMonitorHideThroughput bool `json:"channel_monitor_hide_throughput"`
	// 用户端配额/余额展示默认关闭；管理端始终展示。
