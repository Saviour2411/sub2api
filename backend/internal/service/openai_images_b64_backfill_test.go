...
func TestOpenAIGatewayServiceForwardImages_APIKeyPreservesLocalURLSanitizationWhenBackfillDisabled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","size":"1024x1024"}`)
	...
	// 可选回填关闭不关闭本地隐藏真实上游图片 URL 的强制内联转换。
	require.Len(t, upstream.requests, 2)
	require.Equal(t, http.MethodGet, upstream.requests[1].Method)
	require.True(t, HTTPUpstreamPublicHostsOnly(upstream.requests[1].Context()))
	require.Equal(t, http.StatusOK, rec.Code)
...
func Test本地图片内联下载拒绝私网且不改业务上游策略(t *testing.T) {
	for _, enabled := range []bool{false, true} {
		account := b64BackfillAccount(enabled)
		cfg := &config.Config{}
		cfg.Security.URLAllowlist.AllowInsecureHTTP = true
		cfg.Security.URLAllowlist.AllowPrivateHosts = true
		upstream := &httpUpstreamRecorder{}
		svc := &OpenAIGatewayService{cfg: cfg, httpUpstream: upstream}
		for _, target := range []string{"http://127.0.0.1/a.png", "http://10.0.0.1/a.png", "http://169.254.169.254/a.png"} {
			_, err := svc.downloadOpenAIImageURLToDataURL(context.Background(), account, target, nil)
			require.Error(t, err)
		}
		require.Empty(t, upstream.requests)
		_, err := svc.validateOutboundURL("http://10.0.0.1/v1")
		require.NoError(t, err, "图片下载限制不得禁用业务 HTTP/私网 upstream")
	}
}
