...
type ModelPricing struct {
	...
	MaxReasoningEffortMultiplier       *float64 // 显式 max 推理计费倍率；nil 时不额外加价
	CacheCreation5mPrice               float64  // 5分钟缓存创建每token价格 (USD)
	CacheCreation1hPrice               float64  // 1小时缓存创建每token价格 (USD)
	SupportsCacheBreakdown             bool     // 是否支持详细的缓存分类
	LongContextInputThreshold          int      // 超过阈值后按整次会话提升输入价格
	LongContextThresholdInclusive      bool     // 显式要求时达到阈值即应用；默认保持严格大于以兼容既有模型
...
const (
	// GPT-5.6 长上下文兜底规则仅用于目录缺失字段时的精准修正；
	// GPT-5.4/5.5 等其他模型完全由价格目录或 override 驱动。
...
// maxReasoningEffortBillingMultiplier 仅应用显式配置的倍率，不因模型或推理等级默认加价。
// 思考 Token 已计入输出用量，Fable 5.1 的 max 不应再将全部 Token 费用乘三。
...
// ErrModelPricingUnavailable 表示所有已配置定价源均无法为请求模型提供价格。
...
func (s *BillingService) initFallbackPricing() {
	...
	// Grok 4.20 变体沿用 Grok 4.3 的官方价格和本地长上下文边界语义。
	...
	// Grok 3 Mini 保留历史独立价卡，避免被 Grok 文本模型的通用回退价覆盖。
...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	// 分组与账号开关是双门禁：账号门禁存在时，两者都必须允许。
	contextTierPricingEnabled := !resolved.longContextPricingConfigured || resolved.longContextPricingEnabled
	if input.LongContextBillingEnabled != nil {
		contextTierPricingEnabled = contextTierPricingEnabled && *input.LongContextBillingEnabled
	}

	actualInterval := FindMatchingInterval(resolved.Intervals, totalContext)
	pricingContext := totalContext
	if !contextTierPricingEnabled && actualInterval != nil {
		// 关闭长上下文计费时选择最低档；没有低档时自然回退基础价。
		pricingContext = 1
	}
	...
	// 仅实际采用配置区间时禁用内置长上下文阶梯。区间未命中并回退
	// BasePricing 时仍应用模型自身阶梯，但仍受分组与账号双门禁控制。
	applyLongCtx := contextTierPricingEnabled && actualInterval == nil
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool) *ModelPricing {
	...
	needsLongContextPolicy := isGPT56 &&
		(pricing.LongContextInputThreshold <= 0 || pricing.LongContextInputMultiplier <= 0 || pricing.LongContextOutputMultiplier <= 0)
	...
	if !needsGrokLongContextPolicy && !needsGrok45CacheCorrection && !needsLongContextPolicy && !needsCacheCreationPolicy && fastRatio <= 0 {
		return pricing
	}
	cloned := *pricing
	...
	if needsLongContextPolicy {
		if cloned.LongContextInputThreshold <= 0 {
			cloned.LongContextInputThreshold = openAIGPT56LongContextInputThreshold
		}
		if cloned.LongContextInputMultiplier <= 0 {
			cloned.LongContextInputMultiplier = openAIGPT56LongContextInputMultiplier
		}
		if cloned.LongContextOutputMultiplier <= 0 {
			cloned.LongContextOutputMultiplier = openAIGPT56LongContextOutputMultiplier
		}
	}
	if fastRatio > 0 {
...
func (s *BillingService) knownGrokLongContextFallback(model string) *ModelPricing {
	...
	case "grok-4.3":
		return s.fallbackPrices["grok-4.3"]
	case "grok-4.20-0309-reasoning", "grok-4.20-0309-non-reasoning",
...
// 的标准价倍率：gpt-5.6 系列、gpt-6-astra 与 gpt-5.4 为 2x，gpt-5.5 为 2.5x。未定义 Fast
...
// CalculateCostWithLongContext 保留旧调用兼容，按边际规则计费并关闭目录整单阶梯。
func (s *BillingService) CalculateCostWithLongContext(model string, tokens UsageTokens, rateMultiplier float64, threshold int, extraMultiplier float64) (*CostBreakdown, error) {
	return s.calculateCostWithMarginalLongContext(TokenCostRequest{
		Model:          model,
		Tokens:         tokens,
		RateMultiplier: rateMultiplier,
	}, &LegacyLongContextRule{Threshold: threshold, Multiplier: extraMultiplier})
}
