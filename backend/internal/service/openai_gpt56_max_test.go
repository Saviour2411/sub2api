	...
	for _, requestedModel := range []string{"gpt-6-astra", "astra-public"} {
		for _, stream := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/stream=%t", requestedModel, stream), func(t *testing.T) {
				...
				if requestedModel == "astra-public" {
					// 自定义请求别名须显式配置价格，不得借用转发模型价格完成结算。
					usageService.billingService.pricingService = &PricingService{pricingData: map[string]*LiteLLMModelPricing{
						requestedModel: {InputCostPerToken: 1e-6, OutputCostPerToken: 2e-6},
					}}
				}
				...
				if requestedModel == "astra-public" {
					require.InDelta(t, 5e-6, usageRepo.lastLog.TotalCost, 1e-12)
				}
