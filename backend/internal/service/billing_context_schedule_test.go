...
func scheduleScenarios() []scheduleScenario {
	...
	return []scheduleScenario{
		...
		{
			name: "Grok 超过阈值才进高档", model: "grok-4.5", platform: PlatformGrok, groupPlatform: PlatformGrok,
			group: enabledGroup(PlatformGrok), wantBasis: ContextPricingBasisWholeRequest,
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 2)
				requireTier(t, s.Tiers[0], 0, intPtr(200000), "≤200K", p(2e-6), p(6e-6), nil, p(0.3e-6))
				requireTier(t, s.Tiers[1], 200000, nil, ">200K", p(4e-6), p(12e-6), nil, p(0.6e-6))
			},
		},
		{
