...
func (s *SettingService) IsPreResponseStreamKeepaliveEnabled(ctx context.Context) bool {
	...
	fallback := s.cfg != nil && s.cfg.Gateway.PreResponseStreamKeepaliveEnabled
	if s.settingRepo == nil {
		return fallback
	}
	value, err := s.settingRepo.GetValue(ctx, SettingKeyPreResponseStreamKeepaliveEnabled)
	if err != nil {
		if !errors.Is(err, ErrSettingNotFound) {
			slog.Warn("failed to get pre_response_stream_keepalive_enabled setting", "error", err)
		}
		return fallback
	}
...
func (s *SettingService) GetPreResponseStreamKeepaliveInitialDelay(ctx context.Context) int {
	...
	if s != nil && s.cfg != nil && s.cfg.Gateway.PreResponseStreamKeepaliveInitialDelay >= 0 {
		fallback = s.cfg.Gateway.PreResponseStreamKeepaliveInitialDelay
	}
	if s == nil || s.settingRepo == nil {
		return fallback
	}
	value, err := s.settingRepo.GetValue(ctx, SettingKeyPreResponseStreamKeepaliveInitialDelay)
	if err != nil {
		if !errors.Is(err, ErrSettingNotFound) {
			slog.Warn("failed to get pre_response_stream_keepalive_initial_delay setting", "error", err)
		}
		return fallback
	}
...
func (s *SettingService) getGatewayForwardingSettingsCached(ctx context.Context) gatewayForwardingSettingsResult {
	...
	val, _, _ := gatewayForwardingSF.Do("gateway_forwarding", func() (any, error) {
		...
		if err != nil {
			...
			return gatewayForwardingSettingsResult{openAITTFTMode: OpenAITTFTModeSemantic, fp: true, claudeOAuthSystemPromptInjection: true, rewriteMessageCacheControl: s.defaultRewriteMessageCacheControl(), clientDatelineNormalization: true, semanticErrorConfig: defaultSemanticErrorConfig()}, nil
		}
