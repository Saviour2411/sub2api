...
func (s *OpenAIGatewayService) forwardOpenAIImagesOAuth(
	...
	if parsed.Stream {
		if direct {
			usage, imageCount, imageOutputSizes, firstTokenMs, imageDelivered, err = s.handleOpenAIImagesStreamingResponse(upstreamCtx, resp, c, startTime, account, parsed, parsed)
		} else {
		...
		if err != nil {
			...
			if imageCount > 0 && imageDelivered {
