...
func TestOpenCodeInboundSnapshotDoesNotRetainLargeBody(t *testing.T) {
	c := newOpenCodeSessionTestContext(t, "")
	body := []byte(`{"prompt_cache_key":"original-session","input":"` + strings.Repeat("x", 2<<20) + `"}`)
	rememberOpenCodeInboundBody(c, body)
	snapshots := openCodeInboundBodies(c)
	require.Len(t, snapshots, 1)
	require.Less(t, len(snapshots[0]), 128)
	require.Less(t, cap(snapshots[0]), 256)
	require.NotContains(t, string(snapshots[0]), "input")
	clear(body)
	rememberOpenCodeInboundBody(c, []byte(`{"prompt_cache_key":"converted-session"}`))
	header := make(http.Header)
	applyOpenCodeSessionHeader(c, &Account{Platform: PlatformOpenCodeGo, Type: AccountTypeAPIKey}, "https://opencode.ai/zen/go/v1/messages", header)
	require.Equal(t, "original-session", header.Get(openCodeSessionHeader))
	releaseOpenAIRequestBody(c)
	require.Empty(t, openCodeInboundBodies(c))
}

func TestOpenCodeInboundSnapshotRejectsInvalidSessionAndClearsAtEnd(t *testing.T) {
	c := newOpenCodeSessionTestContext(t, "")
	rememberOpenCodeInboundBody(c, []byte(`{"prompt_cache_key":"bad\r\nheader","input":"discard"}`))
	require.Empty(t, openCodeInboundBodies(c))
	rememberOpenCodeInboundBody(c, []byte(`{"metadata":{"user_id":"stable-session"}}`))
	require.NotEmpty(t, openCodeInboundBodies(c))
...
func TestOpenCodeSessionForwardedByRawChatCompletionsAfterAccountOverride(t *testing.T) {
	...
	resp, attempt, err := svc.sendCCUpstreamRequest(
		context.Background(), c, account,
		"https://opencode.ai/zen/v1/chat/completions", []byte(`{"model":"gpt-5"}`),
		false, "gpt-5", "token", "", "",
	)
	require.NoError(t, err)
	// 非流式请求不启用首 Token 守卫，但仍必须透传 OpenCode 会话头。
