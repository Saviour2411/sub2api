...
func (s *AntigravityGatewayService) TestConnection(ctx context.Context, account *Account, modelID string, prompt ...string) (*TestConnectionResult, error) {
	...
	if len(prompt) > 0 {
		textPrompt = normalizeScheduledTestDefaultPrompt(prompt[0])
	}
	if strings.HasPrefix(modelID, "gemini-") {
		requestBody, err = s.buildGeminiTestRequest(projectID, mappedModel, textPrompt)
	} else {
		requestBody, err = s.buildClaudeTestRequest(projectID, mappedModel, textPrompt)
	}
	if err != nil {
		...
		"contents": []map[string]any{
			{
				...
				"parts": []map[string]any{
					{"text": normalizeScheduledTestDefaultPrompt(prompt)},
				},
			},
		},
...
func (s *AntigravityGatewayService) buildClaudeTestRequest(projectID, mappedModel string, prompt string) ([]byte, error) {
	claudeReq := &antigravity.ClaudeRequest{
		...
		Messages: []antigravity.ClaudeMessage{
			{
				Role:    "user",
				Content: json.RawMessage(strconv.Quote(normalizeScheduledTestDefaultPrompt(prompt))),
			},
		},
		MaxTokens: 1,
		Stream:    false,
