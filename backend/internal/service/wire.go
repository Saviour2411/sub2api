...
func ProvideRateLimitService(
	...
	svc.SetAccountFailureStreakCache(accountFailureStreakCache)
...
func ProvidePaymentService(entClient *dbent.Client, registry *payment.Registry, loadBalancer payment.LoadBalancer, redeemService *RedeemService, subscriptionSvc *SubscriptionService, configService *PaymentConfigService, userRepo UserRepository, groupRepo GroupRepository, affiliateService *AffiliateService, notificationEmailService *NotificationEmailService, settingService *SettingService, userGroupRateRepo UserGroupRateRepository) *PaymentService {
	...
	svc.SetNotificationEmailService(notificationEmailService)
	svc.SetRechargeBonusPolicyDependencies(settingService, userGroupRateRepo)
	return svc
}
...
func ProvideChannelMonitorRunner(
	...
	if svc != nil {
		// 即使 ProvideChannelMonitorService 构造时未注入设置（测试或替代 provider），
		// 也确保运行时读取器已配置。
		svc.SetRuntimeReader(settingService)
		svc.SetScheduler(r)
