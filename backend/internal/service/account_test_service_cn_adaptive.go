...
func (s *AccountTestService) testCNProviderAdaptiveConnection(c *gin.Context, account *Account, modelID string, prompt string) error {
	...
	if err := s.testCNProviderChatCompletionsConnection(c, account, modelID, testPrompt); err != nil {
		return err
	}

	if err := s.testCNProviderAdaptiveAnthropicConnection(c, account, testModelID, authToken, testPrompt); err != nil {
		return err
	}

	if account.SupportsNativeCNResponses() {
		if err := s.testCNProviderAdaptiveResponsesConnection(c, account, testModelID, authToken, testPrompt); err != nil {
			return err
		}
	}
...
func (s *AccountTestService) testCNProviderAdaptiveAnthropicConnection(c *gin.Context, account *Account, testModelID string, authToken string, testPrompt string) error {
	ctx := c.Request.Context()
	...
	payloadBytes, _, err = filterAnthropicSamplingParametersWithSettings(ctx, s.settingService, payloadBytes, testModelID)
	if err != nil {
	...
	if err := s.processCNProviderAdaptiveAnthropicStream(c, resp.Body, account); err != nil {
		return err
	}
...
func (s *AccountTestService) processCNProviderAdaptiveAnthropicStream(c *gin.Context, body io.Reader, account *Account) error {
...
func (s *AccountTestService) testCNProviderAdaptiveResponsesConnection(c *gin.Context, account *Account, testModelID string, authToken string, testPrompt string) error {
	ctx := c.Request.Context()
	...
	payload := createOpenAITestPayload(testModelID, false, testPrompt)
	// DeepSeek / Kimi 原生 Responses 端点是无状态的，不需要 OpenAI 探针的合成 instructions。
	...
	if err := s.processOpenAIStream(c, resp.Body, account); err != nil {
		return err
	}
