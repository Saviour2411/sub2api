...
func TestLegacySchedulerDecision_PreviousResponseRouting(t *testing.T) {
	...
	t.Run("旧来源配置归一为请求模型后保留响应所属账号", func(t *testing.T) {
		released := make([]int64, 0)
		...
		require.NotNil(t, selection)
		require.NotNil(t, selection.Account)
		require.Empty(t, released, "请求模型可用时不能因映射后的上游模型释放槽位")
		releaseLegacySchedulerDecisionSelection(selection)
		require.Equal(t, int64(38102), selection.Account.ID)
		require.Equal(t, openAIAccountScheduleLayerPreviousResponse, decision.Layer)
		require.True(t, decision.StickyPreviousHit)
		require.Equal(t, []int64{38102}, released, "请求结束后仅释放所属账号一次")
		channel, err := svc.channelService.GetChannelForGroup(ctx, groupID)
		require.NoError(t, err)
		require.Equal(t, BillingModelSourceRequested, channel.BillingModelSource)
	})
