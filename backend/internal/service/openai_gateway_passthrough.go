	...
	for {
		...
		if upstreamStream {
			...
			if attemptErr != nil {
				...
				if signal, ok := asOpenAICompactFallbackSignal(attemptErr); ok {
					_ = resp.Body.Close()
					compactResp, compactBody := openAICompactFallbackErrorResponse(resp, signal)
					if s.shouldFailoverOpenAIPassthroughResponse(account, compactResp.StatusCode, compactBody) {
...
func (s *OpenAIGatewayService) handleErrorResponsePassthrough(
	...
	return outcomeError(fmt.Errorf("upstream error: %d (client response sanitized)", resp.StatusCode))
}
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthroughOnAccepted(
	...
	noteTurnStateCommitted := sync.OnceFunc(func() {
		s.noteStagedOpenAICodexTurnStateCommitted(c, account, resp.Header)
	})
	...
	upstreamRequestID := strings.TrimSpace(resp.Header.Get("x-request-id"))
	// flushPending 表示已写入但未到 SSE 空行边界的脏状态。
	flushPending := false
		...
		go func() {
			for {
				...
				case <-keepaliveTicker.C:
					writeMu.Lock()
					if clientDisconnected || flushPending || time.Since(lastDownstreamWriteAt) < keepaliveInterval {
						writeMu.Unlock()
						continue
					}
					if _, err := fmt.Fprint(w, ":\n\n"); err != nil {
						...
						writeMu.Unlock()
						logger.LegacyPrintf("service.openai_gateway", "[OpenAI passthrough] Client disconnected during keepalive ping, continue draining upstream for usage: account=%d", account.ID)
						continue
					}
					flusher.Flush()
					lastDownstreamWriteAt = time.Now()
	...
	writeLineAndFlush := func(line string) bool {
		writeMu.Lock()
		defer writeMu.Unlock()
	...
	ensureResponseFailedTerminal := func() {
		...
		if hit, code, msg := detectOpenAICyberPolicy(bareErrorPayload); hit {
			MarkOpsCyberPolicy(c, CyberPolicyMark{
				Code:           code,
				Message:        msg,
				Body:           truncateString(string(bareErrorPayload), 4096),
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if codexFailureTerminal && sawBareError && !sawResponseFailed &&
				...
				// 后到的成功终态比待定裸 error 更权威，保留其 usage 和终态输出。
				sawBareError = false
				sawFailedEvent = false
				responseFailedPending = false
				suppressCurrentEvent = false
			...
			if eventType == "response.failed" || eventType == "error" {
				...
				cyberPayload := dataBytes
				hit, code, msg := detectOpenAICyberPolicy(cyberPayload)
				if !hit && codexFailureTerminal && eventType == "response.failed" && sawBareError {
					cyberPayload = bareErrorPayload
					hit, code, msg = detectOpenAICyberPolicy(cyberPayload)
				}
				if hit {
					cyberHit = true
					if !codexFailureTerminal || eventType == "response.failed" {
						MarkOpsCyberPolicy(c, CyberPolicyMark{
							Code:           code,
							Message:        msg,
							Body:           truncateString(string(cyberPayload), 4096),
							UpstreamStatus: http.StatusOK,
							UpstreamInTok:  usage.InputTokens,
							UpstreamOutTok: usage.OutputTokens,
						})
		...
		if !clientDisconnected && !failureDelivered && !suppressCurrentEvent {
			...
			if writeLineAndFlush(line) && lineStartsClientOutput {
				noteTurnStateCommitted()
			}
		}
