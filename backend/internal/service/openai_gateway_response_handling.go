...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoningOnAccepted(
	...
	if s.settingService != nil {
		semanticDetector = newSemanticErrorPrefixDetector(s.settingService.GetSemanticErrorConfig(ctx), PlatformOpenAI)
	}
			...
			if semanticDetector != nil && semanticDetector.Enabled() && !openAIStreamClientOutputStarted(c, clientOutputStarted) {
				...
				if !released && sawTerminalEvent && eventType != "response.failed" && (!codexFailureTerminal || eventType != "error") {
					...
					if match := semanticDetector.MatchIfComplete(); match != nil {
						...
						_ = handleSemanticErrorScheduling(s.rateLimitService, ctx, account, match)
						streamEarlyErr = newSemanticErrorFailoverError(match)
						return
					}
				}
			}
