...
// 记录和归档检查点共享写入边界。有数据库时沿用旧实例的 advisory lock，
// 不能改用 Redis 锁，否则蓝绿并行期间会失去跨版本互斥。
func (s *BackupService) lockBackupRecordUpdates(ctx context.Context) (context.Context, func(), error) {
	...
	defer stopWaiting()
	if s.db != nil || lifecycle.Process.SharedDB() != nil {
		release, err := s.lockSharedRecords(waitCtx)
		if err != nil {
			cancel()
			s.recordsMu.Unlock()
