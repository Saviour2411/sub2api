...
func TestResolve_MultiplierOnlyIntervalWithoutBaseRemainsUnavailable(t *testing.T) {
	r := newResolverWithChannel(t, []ChannelModelPricing{{
		Platform: "openai", Models: []string{"unknown-multiplier-model"}, BillingMode: BillingModeToken,
		Intervals: []PricingInterval{{MinTokens: 100, InputMultiplier: testPtrFloat64(2)}},
	}})

	resolved := r.Resolve(context.Background(), PricingInput{
		Model: "unknown-multiplier-model", GroupID: groupIDPtr(),
	})

	require.Equal(t, PricingSourceFallback, resolved.Source)
	require.False(t, resolved.ChannelPricingConfigured)
	require.False(t, resolved.TokenPricingConfigured)
	require.Nil(t, resolved.BasePricing)
