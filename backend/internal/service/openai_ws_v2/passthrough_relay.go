...
func Relay(
	...
	startClientReader := func() {
		...
		go runClientToUpstream(relayCtx, clientConn, options.ReadClientFrame, writeClientFrameUpstream, markActivity, clientToUpstreamFrames, state, onTrace, exitCh)
	}
	...
	if firstExit.stage == "read_client" && firstExit.graceful {
		state.clientDisconnected.Store(true)
	...
	result.ClientDisconnect = state.clientDisconnected.Load()
	...
	for {
		...
		case coderws.MessageBinary:
			...
			if isTerminalEvent(strings.TrimSpace(gjson.GetBytes(payload, "type").String())) {
				...
				openAIWSRelayDeleteActiveTurnTiming(state)
			}
		}
		...
		if writeErr != nil {
			if isDisconnectError(writeErr) && state != nil {
				state.clientDisconnected.Store(true)
			}
			emitRelayTrace(onTrace, RelayTraceEvent{
...
func observeUpstreamMessage(
	...
	if responseID != "" {
		// Bare error 仍可能紧跟同一轮的 authoritative response.failed，
		// 因此不能在 error 帧上提前解除 response ID 与请求模型的绑定。
		requestModel = state.requestModelForResponse(responseID, terminalEvent && eventType != "error")
	}
	...
	if responseID != "" {
		if eventType == "response.failed" && state.pendingBareError != nil && state.pendingBareError.responseID == "" {
			turnTiming = openAIWSRelayBindActiveTurnTiming(state, responseID)
		}
		if turnTiming == nil {
		...
		if turnTiming == nil && eventType == "error" {
			turnTiming = openAIWSRelayGetOrInitTurnTiming(state, "", now)
		}
	}
	observeRelayTurnResponseModel(turnTiming, firstRelayResponseModel(message), terminalEvent)
	if !terminalEvent {
	...
	if eventType == "error" {
		...
		if state.activeTurn != nil && state.activeTurn.requestModel != "" {
			observed.requestModel = state.activeTurn.requestModel
		} else if len(state.pendingModels) > 0 {
			// 先窥视而不消费。若随后到达 response.failed，它仍需用同一
			// 队首模型完成 response ID 绑定；仅 bare error 最终落单时才消费。
			observed.requestModel = state.pendingModels[0]
		}
		if state.activeTurn != nil {
			state.activeTurn.requestModel = observed.requestModel
		}
		state.turnCompletedByError = true
	...
	// response.failed 是 bare error 的权威终态；清除待决事件后只结算一次。
	state.pendingBareError = nil
	state.requestModelMu.Lock()
	state.turnCompletedByError = false
	state.requestModelMu.Unlock()
...
func finalizeObservedRelayTerminal(state *relayState, observed observedUpstreamEvent, now time.Time) observedUpstreamEvent {
	...
	if observed.eventType == "error" {
		...
		if responseID != "" {
			observed.requestModel = state.requestModelForResponse(responseID, true)
		} else {
			state.requestModelMu.Lock()
			if len(state.pendingModels) > 0 {
				observed.requestModel = state.pendingModels[0]
				state.pendingModels = state.pendingModels[1:]
			}
			if observed.requestModel != "" {
	...
	if responseID != "" {
		...
		if turnTiming, ok := openAIWSRelayDeleteActiveTurnTiming(state); ok {
			observed.responseModel = relayTurnResponseModel(&turnTiming)
			observed.responseConflict = turnTiming.responseModelConflict
			observed.responseServiceTier = turnTiming.terminalResponseServiceTier
			observed.startedAt = turnTiming.startAt
			observed.duration = maxDurationZero(now.Sub(turnTiming.startAt))
			observed.firstToken = openAIWSRelayCloneIntPtr(turnTiming.firstTokenMs)
		} else {
			...
			observed.duration = maxDurationZero(now.Sub(startedAt))
		}
		state.lastResponseModel = observed.responseModel
		state.responseConflict = observed.responseConflict
		state.lastResponseServiceTier = observed.responseServiceTier
...
func openAIWSRelayBindActiveTurnTiming(state *relayState, responseID string) *relayTurnTiming {
	if state == nil || state.activeTurn == nil || strings.TrimSpace(responseID) == "" {
		return nil
	}
	if state.turnTimingByID == nil {
		state.turnTimingByID = make(map[string]*relayTurnTiming, 1)
	}
	if timing := state.turnTimingByID[responseID]; timing != nil {
		return timing
	}
	active := state.activeTurn
	for existingID, timing := range state.turnTimingByID {
		if timing == active {
			delete(state.turnTimingByID, existingID)
		}
	}
	state.turnTimingByID[responseID] = active
	return active
}

func openAIWSRelayDeleteActiveTurnTiming(state *relayState) (relayTurnTiming, bool) {
	if state == nil || state.activeTurn == nil {
		return relayTurnTiming{}, false
	}
	timing := state.activeTurn
	for responseID, candidate := range state.turnTimingByID {
		if candidate == timing {
			delete(state.turnTimingByID, responseID)
			break
		}
...
func maxDurationZero(duration time.Duration) time.Duration {
	if duration < 0 {
		return 0
	}
	return duration
