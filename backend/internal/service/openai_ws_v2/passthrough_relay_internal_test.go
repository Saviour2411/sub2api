...
func TestObserveUpstreamMessageErrorThenFailedPreservesFinalTurnContext(t *testing.T) {
	t.Parallel()

	now := time.Unix(350, 0)
	state := newRelayState("gpt-5.6-sol")
	state.setPendingTurnStartedAt(now.Add(-time.Second))
	errorPayload := []byte(`{"type":"error","error":{"code":"server_error","message":"failed"}}`)
	failedPayload := []byte(`{"type":"response.failed","response":{"id":"resp_1","service_tier":"default","usage":{"input_tokens":7,"output_tokens":2}}}`)

	bare := observeUpstreamMessage(state, errorPayload, now.Add(-time.Second), func() time.Time { return now }, nil)
	require.False(t, bare.complete)
	require.False(t, bare.terminal)
	require.Equal(t, "gpt-5.6-sol", bare.requestModel)
	require.Equal(t, errorPayload, bare.payload)

	state.clientDisconnected.Store(true)
	failed := observeUpstreamMessage(state, failedPayload, now.Add(-time.Second), func() time.Time { return now }, nil)
	require.True(t, failed.complete)
	require.True(t, failed.terminal)
	require.Equal(t, "gpt-5.6-sol", failed.requestModel)
	require.Equal(t, failedPayload, failed.payload)
	require.Equal(t, "default", failed.responseServiceTier)
	require.Empty(t, state.turnTimingByID)

	var turns []RelayTurnResult
	emitTurnComplete(func(turn RelayTurnResult) { turns = append(turns, turn) }, state, failed)
	require.Len(t, turns, 1)
	require.Equal(t, "response.failed", turns[0].TerminalEventType)
	require.Equal(t, "gpt-5.6-sol", turns[0].RequestModel)
	require.Equal(t, failedPayload, turns[0].TerminalPayload)
	require.True(t, turns[0].ClientDisconnect)
	require.Equal(t, "default", turns[0].ResponseServiceTier)
}
