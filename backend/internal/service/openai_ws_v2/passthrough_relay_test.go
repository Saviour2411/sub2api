...
func TestRelay_UpstreamErrorEventCompletesExactlyOneTurn(t *testing.T) {
	...
	turns := make([]RelayTurnResult, 0, 2)
	turnStartedAt := time.Date(2026, time.August, 18, 9, 59, 59, 0, time.UTC)
	...
	_, relayExit := Relay(
		...
		RelayOptions{
			FirstTurnStartedAt: turnStartedAt,
			OnTurnComplete:     func(turn RelayTurnResult) { turns = append(turns, turn) },
		},
	)
	...
	require.Len(t, turns, 1)
	require.Equal(t, "response.failed", turns[0].TerminalEventType)
	require.JSONEq(t, `{"type":"response.failed","response":{"id":"resp_after_error","error":{"type":"server_error"}}}`, string(turns[0].TerminalPayload))
	require.Equal(t, "gpt-5", turns[0].RequestModel)
	require.Equal(t, turnStartedAt, turns[0].StartedAt)
	require.Equal(t, "resp_after_error", turns[0].RequestID)
