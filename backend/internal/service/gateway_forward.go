		...
		if err != nil {
			...
			if errors.As(err, &sseErr) {
				...
				return nil, &UpstreamFailoverError{
					StatusCode:             semanticStatus,
					ResponseBody:           body,
					RetryableOnSameAccount: account.IsPoolMode() && account.IsPoolModeRetryableStatus(semanticStatus),
				}
			}
...
// checkChannelPricingRestriction 按用户请求模型检查是否受渠道定价列表限制。
...
func (s *GatewayService) needsUpstreamChannelRestrictionCheck(ctx context.Context, groupID *int64) bool {
	...
	if err != nil {
		slog.Warn("检查渠道上游模型限制失败", "group_id", *groupID, "error", err)
		return false
	}
