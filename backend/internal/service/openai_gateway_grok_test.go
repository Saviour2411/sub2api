...
func TestForwardGrokResponsesCodexAdditionalToolsUsesMixedCacheIntent(t *testing.T) {
	...
	result, err := svc.forwardGrokResponses(context.Background(), c, account, body, "grok", false, false, time.Now())
...
func TestForwardGrokResponsesClaudeDesktopClientToolsUseCacheRoute(t *testing.T) {
	...
	first, err := svc.forwardGrokResponses(context.Background(), newContext(firstBody), account, firstBody, "grok", false, false, time.Now())
	require.NoError(t, err)
	second, err := svc.forwardGrokResponses(context.Background(), newContext(secondBody), account, secondBody, "grok", false, false, time.Now())
	require.NoError(t, err)
...
func TestForwardGrokResponsesStreamingDefaultsEmptyModelTo45AndSnapshots(t *testing.T) {
	...
	result, err := svc.forwardGrokResponses(context.Background(), c, account, body, "", true, true, time.Now())
	require.NoError(t, err)
...
func TestForwardGrokResponsesAPIKeyUsesXAIResponses(t *testing.T) {
	...
	result, err := svc.forwardGrokResponses(context.Background(), c, account, body, "grok", true, true, time.Now())
	require.NoError(t, err)
...
func TestForwardGrokResponsesFailoverKeepsCacheIdentityAcrossAccounts(t *testing.T) {
	...
	_, err := svc.forwardGrokResponses(context.Background(), c, firstAccount, body, "grok", false, false, time.Now())
	var failoverErr *UpstreamFailoverError
	require.ErrorAs(t, err, &failoverErr)

	result, err := svc.forwardGrokResponses(context.Background(), c, secondAccount, body, "grok", false, false, time.Now())
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Len(t, upstream.requests, 2)
	require.Len(t, upstream.bodies, 2)
