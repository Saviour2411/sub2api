...
func (p *postUsageBillingParams) accountQuotaBaseCost() float64 {
	if p != nil && p.AccountQuotaBaseCost != nil {
		return *p.AccountQuotaBaseCost
	}
	if p == nil || p.Cost == nil {
		return 0
	}
...
func notifyAccountQuota(p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if accountQuotaBaseCost <= 0 || p.Account == nil || !p.Account.IsAPIKeyOrBedrock() || deps.balanceNotifyService == nil {
		slog.Debug("notifyAccountQuota: skipped",
			"account_quota_base_cost", accountQuotaBaseCost,
...
func detachStreamUpstreamContext(ctx context.Context, stream bool) (context.Context, context.CancelFunc) {
	...
	return context.WithoutCancel(ctx), lifecycle.Track(lifecycle.Upstream)
}
...
// recordUsageOpts 参数化普通计费与 Gemini 原生入口的边际长上下文计费。
...
// RecordUsageLongContextInput 记录使用量的输入参数，支持 Gemini 原生入口的边际长上下文计费。
...
// RecordUsageWithLongContext 记录使用量并扣费，仅 Gemini /v1beta 使用边际长上下文规则。
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	if err != nil {
		...
		logger.LegacyPrintf("service.gateway", "[SIMPLE MODE] Calculate cost skipped: %v", err)
...
func (s *GatewayService) calculateRecordUsageCost(
	...
	cost, err := s.calculateRecordUsageCostStrict(ctx, result, apiKey, billingModel, multiplier, imageMultiplier, pricingAt, nil)
	if err != nil {
...
// calculateRecordUsageCostStrict 是后扣链路使用的严格版本，定价缺失时必须向上返回错误。
func (s *GatewayService) calculateRecordUsageCostStrict(
	ctx context.Context,
	result *ForwardResult,
	apiKey *APIKey,
	billingModel string,
...
func (s *GatewayService) calculateImageCost(
	...
	if resolved != nil && resolved.Source == PricingSourceChannel {
		...
		if err != nil {
			logger.LegacyPrintf("service.gateway", "Calculate image channel cost failed, using model default: %v", err)
		} else {
			return cost
		}
	}
...
// calculateTokenCostStrict 统一选择分组/渠道定价、旧长上下文规则和内置定价，
// 并把计费错误交给调用方处理，禁止静默按 0 费用继续记录。
func (s *GatewayService) calculateTokenCostStrict(
...
// LegacyLongContextRule 透传计费服务的平台边际长上下文规则。
