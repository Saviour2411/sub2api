...
func compactFirstTokenTestGrokAccount() *Account {
	return &Account{
		ID:          902,
		Name:        "grok-compact-first-token",
...
func compactFirstTokenTestCases() []struct {
	name    string
	account *Account
} {
	return []struct {
		...
		{name: "openai", account: compactFirstTokenTestAccount()},
		{name: "grok", account: compactFirstTokenTestGrokAccount()},
	}
}
...
func TestOpenAIGatewayForwardBodySignalCompactTimesOutBeforeResponseHeaders(t *testing.T) {
	...
	for _, tt := range compactFirstTokenTestCases() {
		t.Run(tt.name, func(t *testing.T) {
			...
			startedAt := time.Now()
			result, err := svc.Forward(c.Request.Context(), c, tt.account, body)
