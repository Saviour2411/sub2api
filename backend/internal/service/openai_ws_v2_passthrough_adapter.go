...
// resolveOpenAIWSPassthroughUpstreamModel 只应用渠道映射。WS 透传不再叠加
// 账号 model_mapping 或 Codex 别名，requestedModel 始终是客户端本轮请求模型。
...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	if hooks != nil && hooks.MapRequestModel != nil {
		...
		channelMappedModel = strings.TrimSpace(mappedModel)
	}
	...
	if channelMappedModel != "" {
		capturedSessionModel = channelMappedModel
	} else {
	...
	policyClientConn := &openAIWSPolicyEnforcingFrameConn{
		...
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			channelMappedModelForThisFrame := ""
			if isResponseCreate {
				...
				if hooks != nil && hooks.MapRequestModel != nil {
					...
					channelMappedModelForThisFrame = strings.TrimSpace(upstreamModel)
				}
			}
			...
			if eventType == "session.update" || eventType == "response.create" {
				mappedModel := ""
				if isResponseCreate && channelMappedModelForThisFrame != "" {
					mappedModel = channelMappedModelForThisFrame
				} else {
	...
	relayResult, relayExit := openaiwsv2.RunEntry(openaiwsv2.EntryInput{
		...
		Options: openaiwsv2.RelayOptions{
			...
			OnTurnComplete: func(turn openaiwsv2.RelayTurnResult) {
				...
				// response.failed 的 turn 回调先于下游写钩子执行，因此必须在
				// AfterTurn 前设置风控标记，确保 handler 能记录事件并封禁会话。
				markOpenAIWSV2PassthroughCyberPolicy(c, turn.TerminalPayload)
			...
			BeforeWriteClient: func(msgType coderws.MessageType, payload []byte, wroteDownstream bool) error {
				...
				if eventType == "error" && markOpenAIWSV2PassthroughCyberPolicy(c, payload) {
					return nil
				}
