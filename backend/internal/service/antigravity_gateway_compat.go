...
func (s *AntigravityGatewayService) consumeAntigravityCompatResponse(
	...
	streamResult, err := s.consumeAntigravityCompatSuccess(c, account, call, resp)
	if err != nil {
		return nil, err
	}
...
func (s *AntigravityGatewayService) consumeAntigravityCompatSuccess(
	...
	if call.request.protocol == antigravityCompatChatCompletions {
		return s.handleChatCompletionsNonStreamingFromAntigravity(c, resp, account, call.request.startTime, call.request.originalModel)
	}
	return s.handleResponsesNonStreamingFromAntigravity(c, resp, account, call.request.startTime, call.request.originalModel)
}
...
func (s *AntigravityGatewayService) handleChatCompletionsNonStreamingFromAntigravity(
	...
	claudeResponse, result, err := s.collectClaudeStreamResponse(c, resp, account, startTime, originalModel)
	if err != nil {
