...
func TestAccountTestService_OpenCodeGoChatMappingOnlyOnce(t *testing.T) {
	account := openCodeGoTestAccount(407)
	account.Credentials["model_mapping"] = map[string]any{"alias": "glm-5.3", "glm-5.3": "wrong-second-model"}
	svc, upstream := adaptiveCNAccountTestService(account, adaptiveCNChatTestResponse())
	c, _ := newTestContext()
	err := svc.TestAccountConnection(c, account.ID, "alias", "本地测试提示词", AccountTestModeDefault)
	require.NoError(t, err)
	require.Len(t, upstream.requests, 1)
	require.Equal(t, "glm-5.3", gjson.GetBytes(upstream.lastBody, "model").String())
	require.Contains(t, string(upstream.lastBody), "本地测试提示词")
	require.Equal(t, "wrong-second-model", account.GetMappedModel("glm-5.3"))
}
