...
// 默认规范化保留缓存断点；本地调用方仍可显式启用 system 缓存清洗。
...
func TestNormalizeClaudeOAuthRequestBodyKeepsLocalOptionsIndependent(t *testing.T) {
	body := []byte(`{"model":"custom-model","system":[{"type":"text","text":"You are OpenCode, the best coding agent on the planet.","cache_control":{"type":"ephemeral"}}],"messages":[]}`)
	out, model := normalizeClaudeOAuthRequestBody(body, "custom-model", claudeOAuthNormalizeOptions{
