...
func TestAccountTestService_AdaptiveChatOnlyProvidersTestChatAndAnthropicEndpoints(t *testing.T) {
	for index, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			settingService := testSemanticSettingService(t, testCase.platform, "never-match-adaptive-test", "")
			gatewaySettings := DefaultGatewaySettings()
			gatewaySettings.AnthropicSamplingParameterFilterEnabled = true
			gatewaySettings.AnthropicSamplingParameterFilterModels = []string{testCase.model}
			settingService.storeGatewaySettingsCache(gatewaySettings, time.Hour)
...
func TestAccountTestService_AdaptiveAnthropicSemanticErrorFails(t *testing.T) {
	account := adaptiveCNAccountTestAccount(306, PlatformKimi)
	semanticResponse := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}},
		Body: io.NopCloser(strings.NewReader(`data: {"type":"content_block_delta","delta":{"text":"join the new community"}}

data: {"type":"message_stop"}

`)),
	}
	svc, upstream := adaptiveCNAccountTestService(account, adaptiveCNChatTestResponse(), semanticResponse)
	svc.settingService = testSemanticSettingService(t, PlatformKimi, "join the new community", "semantic blocked")
	c, recorder := newTestContext()

	err := svc.TestAccountConnection(c, account.ID, "kimi-k2.5", "hello", AccountTestModeDefault)
