...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if err != nil {
		...
		case cnPricingCandidatesFiltered && isUsagePricingUnavailableError(err):
			...
			).Warn("openai_usage.cn_pricing_missing_record_zero_cost", zap.Error(err))
			cost = &CostBreakdown{BillingMode: string(BillingModeToken)}
		default:
			return err
		}
	}
	// 上游响应模型只用于使用日志和 mismatch 诊断。普通分组继续严格按用户请求模型
	// 计费，不能由上游成功响应反向改写用户扣费来源。
	...
	if billingErr != nil {
		...
		writeUsageLogBestEffort(ctx, s.usageLogRepo, usageLog, "service.openai_gateway")
		if pricingErr != nil {
			return errors.Join(pricingErr, billingErr)
		}
		return billingErr
	}
	writeUsageLogBestEffort(ctx, s.usageLogRepo, usageLog, "service.openai_gateway")
