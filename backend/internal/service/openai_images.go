...
func (s *OpenAIGatewayService) forwardOpenAIImagesAPIKey(
	...
	if parsed.Stream && isEventStreamResponse(resp.Header) {
		...
		imageDelivered = streamDelivered
		...
		nonStreamUsage, nonStreamCount, nonStreamSizes, nonStreamDelivered, err := s.handleOpenAIImagesNonStreamingResponse(upstreamCtx, resp, c, account, parsed)
		if err != nil {
			return nil, err
		}
...
func (s *OpenAIGatewayService) handleOpenAIImagesNonStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, parsed *OpenAIImagesRequest) (OpenAIUsage, int, []string, bool, error) {
	body, err := ReadUpstreamResponseBody(resp.Body, s.cfg, c, openAITooLargeError)
	if err != nil {
		return OpenAIUsage{}, 0, nil, false, err
	}
	body, err = s.normalizeOpenAIImagesResponseForClient(ctx, account, body)
	if err != nil {
		return OpenAIUsage{}, 0, nil, false, err
	}
	...
	return usage, imageCount, collectOpenAIResponseImageOutputSizesFromJSONBytes(body), imageCount > 0 && c.Writer.Size() > 0, nil
}
...
func (s *OpenAIGatewayService) handleOpenAIImagesStreamingResponse(
	...
	if direct == nil && shouldNormalizeOpenAIImagesResponse(parsed) {
	...
	processSSEData := func(dataBytes []byte) {
		...
		if !clientDisconnected {
			if err := s.writeOpenAIImagesStreamEvent(c, flusher, eventType, dataBytes); err != nil {
				...
				deliveredCounter.AddSSEData(dataBytes)
				lastDownstreamWriteAt = time.Now()
			}
		}
	}
		...
		if !clientDisconnected && direct == nil {
			if _, writeErr := c.Writer.Write(line); writeErr != nil {
				...
				if data, ok := extractOpenAISSEDataLine(trimmedLine); ok {
					deliveredCounter.AddSSEData([]byte(data))
				}
				flusher.Flush()
				lastDownstreamWriteAt = time.Now()
			}
	...
	if streamInterval <= 0 && keepaliveInterval <= 0 {
		...
		for {
			...
			if err != nil {
				flushSSEEvent()
				return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, deliveredCounter.Count() > 0, err
			}
		}
		flushSSEEvent()
		finalizeFallbackBody()
		return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, deliveredCounter.Count() > 0, finish()
	}
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.err != nil {
				flushSSEEvent()
				return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, deliveredCounter.Count() > 0, ev.err
			}
			processLine(ev.line)
		case <-intervalCh:
			...
			if clientDisconnected {
				return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, deliveredCounter.Count() > 0, fmt.Errorf("image stream incomplete after timeout")
			}
			...
			return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, deliveredCounter.Count() > 0, fmt.Errorf("image stream data interval timeout")
		case <-keepaliveCh:
...
func shouldNormalizeOpenAIImagesResponse(parsed *OpenAIImagesRequest) bool {
	return parsed != nil
}

func (s *OpenAIGatewayService) normalizeOpenAIImagesResponseForClient(ctx context.Context, account *Account, body []byte) ([]byte, error) {
	if len(body) == 0 || !gjson.ValidBytes(body) {
		return body, nil
	}
	rewritten, _, err := s.normalizeOpenAIImagesJSON(ctx, account, body)
	return rewritten, err
}

func (s *OpenAIGatewayService) normalizeOpenAIImagesJSON(ctx context.Context, account *Account, body []byte) ([]byte, bool, error) {
	if len(body) == 0 || !gjson.ValidBytes(body) {
	...
	changed, err := s.normalizeOpenAIImagesJSONNode(ctx, account, decoded)
	if err != nil {
...
func (s *OpenAIGatewayService) normalizeOpenAIImagesJSONNode(ctx context.Context, account *Account, node any) (bool, error) {
	switch value := node.(type) {
	case map[string]any:
		changed := false
		imageChanged, err := s.normalizeOpenAIImageObject(ctx, account, value)
		if err != nil {
			return false, err
		}
		changed = changed || imageChanged
		for _, child := range value {
			childChanged, err := s.normalizeOpenAIImagesJSONNode(ctx, account, child)
			if err != nil {
				return false, err
			}
...
func (s *OpenAIGatewayService) normalizeOpenAIImageObject(ctx context.Context, account *Account, imageObj map[string]any) (bool, error) {
	if imageObj == nil {
		return false, nil
	}
	rawURL, hasURL := imageObj["url"].(string)
	rawB64, hasB64 := imageObj["b64_json"].(string)
	normalizedB64 := normalizeOpenAIImageBase64(rawB64)
	hasUsableB64 := hasB64 && normalizedB64 != ""

	if hasURL && shouldSanitizeOpenAIImageURL(rawURL) {
		dataURL, err := s.openAIImageURLToDataURL(ctx, account, rawURL, imageObj)
		if err != nil {
			if !hasUsableB64 {
				return false, err
			}
			dataURL = openAIImageDataURLFromInlineB64(imageObj)
		}
		if extracted := extractBase64FromDataURL(dataURL); extracted != "" {
			imageObj["b64_json"] = extracted
		}
		imageObj["url"] = dataURL
		return true, nil
	}
	if hasUsableB64 {
		dataURL := openAIImageDataURLFromInlineB64(imageObj)
		changed := false
		if rawB64 != normalizedB64 {
			imageObj["b64_json"] = normalizedB64
			changed = true
		}
		if !hasURL || strings.TrimSpace(rawURL) != dataURL {
			imageObj["url"] = dataURL
			changed = true
		}
		return changed, nil
...
func extractBase64FromDataURL(dataURL string) string {
	...
	idx := strings.Index(strings.ToLower(trimmed), ";base64,")
	if idx < 0 {
		return ""
	}
	return strings.TrimSpace(trimmed[idx+len(";base64,"):])
}

func shouldSanitizeOpenAIImageURL(raw string) bool {
	...
	return strings.HasPrefix(strings.ToLower(trimmed), "http://") ||
		strings.HasPrefix(strings.ToLower(trimmed), "https://")
}

func (s *OpenAIGatewayService) openAIImageURLToDataURL(ctx context.Context, account *Account, rawURL string, imageObj map[string]any) (string, error) {
	rawURL = strings.TrimSpace(rawURL)
	if fallback := openAIImageDataURLFromInlineB64(imageObj); fallback != "" {
		dataURL, err := s.downloadOpenAIImageURLToDataURL(ctx, account, rawURL, imageObj)
		if err == nil {
			return dataURL, nil
		}
		return fallback, nil
	}
	return s.downloadOpenAIImageURLToDataURL(ctx, account, rawURL, imageObj)
}

func openAIImageDataURLFromInlineB64(imageObj map[string]any) string {
	if imageObj == nil {
		return ""
	}
	rawB64, _ := imageObj["b64_json"].(string)
	normalized := normalizeOpenAIImageBase64(rawB64)
	if normalized == "" {
		return ""
	}
	mimeType := openAIImageMIMETypeFromImageObject(imageObj, "")
	return "data:" + mimeType + ";base64," + normalized
}

func (s *OpenAIGatewayService) downloadOpenAIImageURLToDataURL(ctx context.Context, account *Account, rawURL string, imageObj map[string]any) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("download image url: %w", err)
	}
	defer func() {
	...
	if resp == nil {
		return "", fmt.Errorf("download image url: empty response")
	}
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		return "", fmt.Errorf("download image url: status %d", resp.StatusCode)
	}
	...
	if len(data) == 0 {
		return "", fmt.Errorf("download image url: empty image")
	}
	...
	if !strings.HasPrefix(strings.ToLower(mimeType), "image/") {
		mimeType = openAIImageMIMETypeFromImageObject(imageObj, http.DetectContentType(data))
	}
	return "data:" + mimeType + ";base64," + base64.StdEncoding.EncodeToString(data), nil
}

func openAIImageMIMETypeFromImageObject(imageObj map[string]any, fallback string) string {
	for _, key := range []string{"mime_type", "mimeType", "content_type", "output_format"} {
		if imageObj == nil {
			break
		}
		if value, ok := imageObj[key].(string); ok {
			mimeType := openAIImageOutputMIMEType(strings.TrimSpace(value))
			if strings.HasPrefix(strings.ToLower(mimeType), "image/") {
				return mimeType
			}
		}
	}
	if strings.HasPrefix(strings.ToLower(strings.TrimSpace(fallback)), "image/") {
		return strings.TrimSpace(fallback)
	}
	return "image/png"
}

func (s *OpenAIGatewayService) handleOpenAIImagesStreamingURLResponse(
	ctx context.Context,
	resp *http.Response,
	c *gin.Context,
	startTime time.Time,
	...
	writeEvent := func(eventName string, data []byte) error {
		seenSSEData = true
		fallbackBody.Reset()
		fallbackBytes = 0
		if len(data) > 0 && gjson.ValidBytes(data) {
			...
			rewritten, _, err := s.normalizeOpenAIImagesJSON(ctx, account, data)
			if err != nil {
				return err
			}
			data = rewritten
	...
	processLine := func(line []byte) error {
		...
		complete, eventName, data := sseEvent.AddLine(string(line))
		if !complete {
			trimmedLine := strings.TrimRight(string(line), "\r\n")
			if _, ok := extractOpenAISSEDataLine(trimmedLine); !ok && strings.TrimSpace(trimmedLine) != "" && !seenSSEData && !fallbackTooLarge {
				fallbackBytes += int64(len(line))
				if fallbackBytes <= fallbackLimit {
					_, _ = fallbackBody.Write(line)
				} else {
	...
	finalize := func() error {
		if complete, eventName, data := sseEvent.Flush(); complete {
			return writeEvent(eventName, data)
		}
		if !seenSSEData && fallbackBody.Len() > 0 {
			body := bytes.TrimSpace(fallbackBody.Bytes())
			...
			mergeOpenAIUsage(&usage, body)
			imageCounter.AddJSONResponse(body)
			rewritten, err := s.normalizeOpenAIImagesResponseForClient(ctx, account, body)
			if err != nil {
				return err
			}
			if !clientDisconnected {
	...
	if streamInterval <= 0 && keepaliveInterval <= 0 {
		...
		for {
			...
			if processErr := processLine(line); processErr != nil {
				return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, deliveredCounter.Count() > 0, processErr
			}
			if err == io.EOF {
				break
			}
			if err != nil {
				if finalizeErr := finalize(); finalizeErr != nil {
					return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, deliveredCounter.Count() > 0, finalizeErr
				}
				return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, deliveredCounter.Count() > 0, err
			}
		}
		...
		return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, deliveredCounter.Count() > 0, nil
	}
	...
	for {
		...
		case <-intervalCh:
			...
			logger.LegacyPrintf("service.openai_gateway", "[OpenAI] Images sanitized stream data interval timeout: interval=%s", streamInterval)
			_ = s.writeOpenAIImagesStreamEvent(c, flusher, "error", buildOpenAIImagesStreamErrorBody(fmt.Sprintf("upstream image stream idle for %s", streamInterval)))
			return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, deliveredCounter.Count() > 0, fmt.Errorf("image stream data interval timeout")
		case <-keepaliveCh:
			if clientDisconnected || time.Since(lastDownstreamWriteAt) < keepaliveInterval {
...
type openAIImagesSSEEventBuffer struct {
	eventName string
	dataLines []string
}

func (b *openAIImagesSSEEventBuffer) AddLine(line string) (bool, string, []byte) {
	...
	if data, ok := extractOpenAISSEDataLine(trimmed); ok {
		b.dataLines = append(b.dataLines, data)
		return false, "", nil
	}
...
func (b *openAIImagesSSEEventBuffer) Flush() (bool, string, []byte) {
	...
	return true, eventName, []byte(strings.TrimSpace(data))
}
