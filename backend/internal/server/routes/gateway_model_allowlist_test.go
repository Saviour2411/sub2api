...
func TestGatewayRoutesGroupModelAllowlistMountedOnEveryGatewayRoute(t *testing.T) {
	...
	rootHelper := regexp.MustCompile(regexp.QuoteMeta(`handlers := []gin.HandlerFunc{limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic}`))
	require.Regexp(t, rootHelper, source,
		"root alias helper must place the allowlist between apiKeyAuth and compositeTarget")
	require.Regexp(t, regexp.MustCompile(`handlers = append\(handlers, extra\.\.\.\)\s+handlers = append\(handlers, handler\)\s+r.Handle\(method, path, handlers\.\.\.\)`), source,
		"根路径别名必须在准入链之后追加本地统计中间件和业务处理器")
	...
	codexDirect := regexp.MustCompile(regexp.QuoteMeta(`codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, trackImageGroupResult)`))
