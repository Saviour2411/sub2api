...
func RegisterGatewayRoutes(
	...
	// 之前，避免逐条路由手工维护链导致漏挂。
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc, extra ...gin.HandlerFunc) {
		handlers := []gin.HandlerFunc{limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic}
		handlers = append(handlers, extra...)
		handlers = append(handlers, handler)
		r.Handle(method, path, handlers...)
	}
	rootRoute(http.MethodPost, "/responses", bodyLimit, responsesHandler, trackImageGroupResult)
	rootRoute(http.MethodPost, "/responses/*subpath", bodyLimit, guardResponsesSubpath(responsesHandler), trackImageGroupResult)
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, trackImageGroupResult)
	{
	...
	rootRoute(http.MethodPost, "/images/generations", bodyLimit, imagesHandler, trackImageGroupResult)
	rootRoute(http.MethodPost, "/images/edits", bodyLimit, imagesHandler, trackImageGroupResult)
	rootRoute(http.MethodPost, "/images/generations/async", bodyLimit, h.AsyncImage.Submit, trackImageGroupResult)
	rootRoute(http.MethodPost, "/images/edits/async", bodyLimit, h.AsyncImage.Submit, trackImageGroupResult)
	rootRoute(http.MethodGet, "/images/tasks/:task_id", bodyLimit, h.AsyncImage.Get)
	rootRoute(http.MethodPost, "/videos", bodyLimit, videoGenerationHandler, trackImageGroupResult)
	rootRoute(http.MethodPost, "/videos/generations", bodyLimit, videoGenerationHandler, trackImageGroupResult)
	rootRoute(http.MethodPost, "/videos/edits", bodyLimit, videoEditHandler, trackImageGroupResult)
	rootRoute(http.MethodPost, "/videos/extensions", bodyLimit, videoExtensionHandler, trackImageGroupResult)
