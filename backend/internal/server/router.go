...
func SetupRouter(
	...
	if web.HasEmbeddedFrontend() {
		frontendServer, err := web.NewFrontendServer(settingService) //nolint:staticcheck // 非 embed 构建恒返回错误；embed 构建仍需处理真实初始化失败。
		if err != nil {                                              //nolint:staticcheck // 非 embed 构建恒返回错误；embed 构建仍需处理真实初始化失败。
