...
const DefaultCSPPolicy = "default-src 'self'; script-src 'self' __CSP_NONCE__ https://challenges.cloudflare.com https://static.cloudflareinsights.com https://*.stripe.com https://static.airwallex.com https://checkout.airwallex.com https://static-demo.airwallex.com https://checkout-demo.airwallex.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://static.airwallex.com https://checkout.airwallex.com https://static-demo.airwallex.com https://checkout-demo.airwallex.com; img-src 'self' data: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https:; worker-src 'self' blob:; frame-src https://challenges.cloudflare.com https://*.stripe.com https://checkout.airwallex.com https://checkout-demo.airwallex.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"
...
type GatewayConfig struct {
	...
	// PreResponseStreamKeepaliveEnabled: 流式请求等待上游响应头阶段是否提前打开 SSE 并发送心跳
	PreResponseStreamKeepaliveEnabled bool `mapstructure:"pre_response_stream_keepalive_enabled"`
	// PreResponseStreamKeepaliveInitialDelay: 首次预响应心跳延迟（秒）
	PreResponseStreamKeepaliveInitialDelay int `mapstructure:"pre_response_stream_keepalive_initial_delay"`
...
func setDefaults() {
	...
	viper.SetDefault("gateway.pre_response_stream_keepalive_enabled", false)
	viper.SetDefault("gateway.pre_response_stream_keepalive_initial_delay", 80)
...
func (c *Config) Validate() error {
	...
	if c.Gateway.PreResponseStreamKeepaliveInitialDelay < 0 {
		return fmt.Errorf("gateway.pre_response_stream_keepalive_initial_delay must be non-negative")
	}
	if c.Gateway.PreResponseStreamKeepaliveEnabled &&
		(c.Gateway.PreResponseStreamKeepaliveInitialDelay < 5 || c.Gateway.PreResponseStreamKeepaliveInitialDelay > 110) {
		return fmt.Errorf("gateway.pre_response_stream_keepalive_initial_delay must be between 5-110 seconds when enabled")
	}
