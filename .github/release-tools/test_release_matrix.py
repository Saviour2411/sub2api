...
class LocalReleaseWorkflowTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_dockerhub_credentials_keep_secrets_or_variables_fallback(self):
        env = self.jobs['release']['env']
        self.assertEqual(env['DOCKERHUB_USERNAME'], '${{ secrets.DOCKERHUB_USERNAME || vars.DOCKERHUB_USERNAME }}')
        self.assertEqual(env['DOCKERHUB_TOKEN_AVAILABLE'], "${{ secrets.DOCKERHUB_TOKEN != '' }}")
        for name in ('Login to DockerHub', 'Update DockerHub description'):
            step = self.step('release', name)
            self.assertEqual(step['if'], "${{ env.DRY_RUN != 'true' && env.SIMPLE_RELEASE != 'true' && env.DOCKERHUB_USERNAME != '' && env.DOCKERHUB_TOKEN_AVAILABLE == 'true' }}")
            self.assertEqual(step['with']['username'], '${{ env.DOCKERHUB_USERNAME }}')
        image = self.step('release', 'Build images and publish manifests')
        self.assertEqual(image['env']['DOCKERHUB_USERNAME'], "${{ env.DOCKERHUB_TOKEN_AVAILABLE == 'true' && env.DOCKERHUB_USERNAME || 'skip' }}")

    def test_dry_run_cannot_write_version_or_deploy(self):
        sync = self.jobs['sync-version-file']
        self.assertEqual(sync['needs'], ['prepare', 'release'])
        self.assertEqual(sync['permissions'], {'contents': 'write'})
        self.assertEqual(sync['if'], "${{ needs.release.result == 'success' && needs.prepare.outputs.dry_run != 'true' }}")
        deploy = self.jobs['deploy-production']
        self.assertEqual(deploy['needs'], ['prepare', 'release'])
        self.assertEqual(deploy['if'], "${{ needs.release.result == 'success' && needs.prepare.outputs.dry_run != 'true' && needs.prepare.outputs.simple != 'true' && (github.event_name == 'push' || github.event.inputs.deploy_production == 'true') }}")
        self.assertEqual(self.workflow['on']['workflow_dispatch']['inputs']['deploy_production']['default'], 'true')

    def test_all_artifacts_and_deployment_use_prepared_commit(self):
        for job in ('build-frontend', 'build-binaries', 'release', 'deploy-production'):
        ...
        self.assertEqual(self.step('deploy-production', 'Resolve immutable release')['env']['RELEASE_TAG'], '${{ needs.prepare.outputs.tag }}')

    def test_canvas_build_and_local_release_timeouts_are_preserved(self):
        canvas = self.step('build-frontend', 'Typecheck and build canvas')
        self.assertEqual(canvas['working-directory'], 'canvas')
        self.assertEqual(canvas['run'], 'pnpm run build')
        for job in ('build-binaries', 'release'):
            build = next(step for step in self.jobs[job]['steps'] if step.get('uses', '').startswith('goreleaser/'))
            self.assertIn('--timeout 120m', build['with']['args'])
        deploy = self.jobs['deploy-production']
        self.assertEqual(deploy['timeout-minutes'], '120')
        command = self.step('deploy-production', 'Blue-green deploy and observe natural drain')['run']
        self.assertIn('--window 3600 --force-after-window --legacy-window 1200 --legacy-quiet 900', command)
