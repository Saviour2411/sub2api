...
export function useAgentBridge(params: AgentBridgeParams) {
    const { projectId, title, nodes, connections, selectedNodeIds, viewport, nodesRef, connectionsRef, selectedNodeIdsRef, viewportRef, generateNodeRef, setNodes, setConnections, setSelectedNodeIds, setSelectedConnectionId, setViewport, setContextMenu } =
        params;
    const setCanvasAssistantContext = useCanvasAssistantContextStore((state) => state.setContext);
    ...
    useEffect(() => {
        setCanvasAssistantContext({ snapshot: agentSnapshot, applyOps: applyAgentOps, undoOps: undoAgentOps, canUndo: Boolean(agentUndoSnapshot) });
        return () => setCanvasAssistantContext(null);
    }, [agentSnapshot, applyAgentOps, agentUndoSnapshot, setCanvasAssistantContext, undoAgentOps]);
