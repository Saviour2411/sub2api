...
export function CanvasSidePanel({ nodes, selectedNodeIds, onFocusNode, onPreviewNode, onInsertAsset }: Props) {
    ...
    return (
        ...
        >
            <motion.aside
                className="canvas-side-panel relative flex h-full shrink-0 flex-col overflow-hidden border-r"
                initial={{ x: -48 }}
                animate={{ x: panelClosing ? -28 : 0 }}
                transition={{ duration: resizing ? 0 : PANEL_MOTION_SECONDS, ease: PANEL_EASE }}
                style={{ width, maxWidth: "calc(100vw - 48px)", background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.text }}
                data-canvas-no-zoom
            >
                <div className="flex items-center gap-5 px-4 pt-3.5 max-sm:pr-12">
                ...
                <button
                    type="button"
                    className="absolute right-2 top-2 z-50 hidden size-8 place-items-center rounded-md transition hover:bg-black/10 max-sm:grid dark:hover:bg-white/10"
                    onClick={togglePanel}
                ...
                <button type="button" className="absolute inset-y-0 right-0 z-40 w-4 translate-x-1/2 cursor-col-resize max-sm:hidden" onPointerDown={startResize} aria-label={t("canvas.sidePanel.resize")} />
            </motion.aside>
        </motion.div>
    );
}
