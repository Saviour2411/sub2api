...
## 2026-07-13 无操作同步检查 e316ebf52

- 执行时间：2026-07-13T22:59:10+08:00
- 执行状态：成功；上游无新增提交，未执行 merge 或 cherry-pick，仅追加本记录
- 本地目标分支：`main`
- `LOCAL_PRE_SYNC_SHA`：`39375fe7b2ef62e762de8c089c7609ae524fe853`
- 上游代码合并提交：无；固定目标 SHA 已是本地祖先
- 上游仓库：`https://github.com/Wei-Shaw/sub2api.git`
- 上游分支：`main`
...
- `LAST_FULLY_INTEGRATED_UPSTREAM_SHA`：`e316ebf52838a89d57fc790981cce7520f819ac8`
- 集成策略：无操作同步；确认固定上游目标已经完整集成，不创建代码合并提交
- 备份分支：`backup/pre-upstream-sync-20260713-224801-39375fe7b`
- 同步分支：`sync/upstream-20260713-e316ebf52`

### 上游提交处置

固定范围 `e316ebf52838a89d57fc790981cce7520f819ac8..e316ebf52838a89d57fc790981cce7520f819ac8` 共 0 个提交，无 `Applied`、`Skipped`、`Deferred` 或 `Conflict`。固定目标 SHA 已在 2026-07-12 的完整同步中集成，本次按 `Already Applied` 的无操作目标处理，不重复创建提交映射。

### 本地提交与文件

- 无上游代码提交映射，无兼容性修改。
- 本次代码树相对 `LOCAL_PRE_SYNC_SHA` 保持不变。
- 仅修改 `docs/upstream-sync-history.md`；本记录提交自身的 SHA 在最终总结中报告。

### 冲突与用户决定

- 没有 Git 文本冲突、未合并索引或语义冲突。
- 用户于集中审批后回复“批准执行”，批准创建隔离分支、运行验证并追加无操作同步记录。
- 未发生计划外业务取舍，未删除、覆盖或回退二次开发行为。

### 刻意保留的二次开发功能

- 全局 Anthropic Claude Code 上游模拟、请求清洗及相关配置入口。
- 网关设置单调 revision 保存、账号连续失败停调度和失败状态清理。
- 首 Token 超时、流式响应包装、失败切号及调度兼容。
- Codex 图片工具禁用、namespace 图片工具剥离及工具重写策略。
- 按用户请求模型计费、网关配置、渠道及 Image 分组成功率监控。
- 高级调度、本地审计、批量图片、签到、模型广场和充值返利等二次开发功能。

### 验证记录

验证使用 Go 1.26.5、Node 24.15.0 和 pnpm 9.15.9。同步前后代码树相同；同步前 unit 全量运行出现一个可单独通过的顺序或并发敏感失败，同步后全量运行通过，因此没有同步后新增失败。

| 阶段 | 命令 | 退出码 | 结果 |
| --- | --- | ---: | --- |
| 同步前 | `go test -tags=unit ./...` | 1 | `TestProcessGeminiStream_SemanticErrorFails` 一次预期错误却得到 nil；其余包通过 |
| 同步前 | `go test -tags=unit ./internal/service -run '^TestProcessGeminiStream_SemanticErrorFails$' -count=1 -v` | 0 | 目标用例单独重跑通过 |
| 同步前 | `go test -tags=integration ./...` | 1 | 仅 `internal/pkg/tlsfingerprint` 的 3 个用例因 `tls.peet.ws:443` 拒绝连接失败，其余通过 |
| 同步前 | `golangci-lint run ./...` | 0 | 0 issues |
| 同步前 | `go build -o <临时目录>/sub2api-baseline.exe ./cmd/server` | 0 | 通过，临时产物已删除 |
| 同步前 | `corepack pnpm --dir frontend run lint:check` | 0 | 通过 |
| 同步前 | `corepack pnpm --dir frontend run typecheck` | 0 | 通过 |
| 同步前 | CI 关键 Vitest 集合 | 0 | 6 个文件、97 个测试通过 |
| 同步前 | `corepack pnpm --dir frontend run build` | 0 | 通过，存在既有 chunk、动态导入和 Browserslist 警告 |
| 同步后 | `go test -tags=unit ./...` | 0 | 全量通过，包括同步前偶发失败的目标用例 |
| 同步后 | `go test -tags=integration ./...` | 1 | 与同步前相同，仅 3 个外部联网用例失败，无新增失败 |
| 同步后 | `golangci-lint run ./...` | 0 | 0 issues |
| 同步后 | `go build -o <临时目录>/sub2api-post.exe ./cmd/server` | 0 | 通过，临时产物已删除 |
| 同步后 | `corepack pnpm --dir frontend run lint:check` | 0 | 通过 |
| 同步后 | `corepack pnpm --dir frontend run typecheck` | 0 | 通过 |
| 同步后 | CI 关键 Vitest 集合 | 0 | 6 个文件、97 个测试通过 |
| 同步后 | `corepack pnpm --dir frontend run build` | 0 | 通过，警告与同步前一致 |
| 同步前后 | `git diff --check`、工作树和未合并索引检查 | 0 | 代码验证期间工作树保持干净，无冲突或生成文件变更 |

### 未验证项与残余风险

- 本机没有 Node 20 环境，未验证与 CI Node 20 完全等价；Node 24.15.0 下验证通过。
- 未读取 `.env`，没有安全的隔离运行配置，因此未执行本地服务启动和健康检查。
- TLS 指纹集成测试依赖外部站点，本次环境无法访问；同步前后失败集合一致。
- 同步前 unit 全量运行存在一次顺序或并发敏感失败；目标用例单独重跑及同步后全量运行均通过，未在本次无操作同步中修改范围外代码。
- 未执行 push、PR、部署、远程服务器访问或生产数据操作。
