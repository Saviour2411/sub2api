...
- 2026-09-23 为 `CUST-OPS-008` 补充构建内 `backend/internal/pkg/buildmeta/upstream-sync.json`，供管理员查看已同步原版版本。初始记录沿用2026-09-20的完整同步基线 `0.2.7 / 7c700729c23187d31ed320f6b19c790e2f194826`，同步时间为 `2026-09-20T23:09:09+08:00`。
...
## 2026-09-24 239/240专项发布适配（北京时间）

- 上述同步记录的本地操作边界是当时状态。用户随后授权推送、CI后合入主线、创建tag并经Actions自动蓝绿部署。同步提交`304006b24fcc7a4b55349031955f8738f3e244d9`已推送并快进远端main；候选CI/安全运行`35920884082`/`35920884065`及主线`35922185867`/`35922185846`均成功，每轮8项CI与2项安全任务未跳过。
- 生产只读预检发现新增239/240被原门禁拒绝；未创建必然失败的tag，也未修改生产。用户获知旧计费键转换与索引风险后明确批准专项兼容适配、隔离测试及后续发布。专项分支为`codex/bluegreen-239-240-20260924`，计划新tag`v0.1.246`，不移动旧标签。
- 关联`CUST-OPS-007`并同步更新二开清单与变更记录。新增固定旧SHA/SQL/旧代码摘要的239/240审批、PostgreSQL18及16MiB普通表限制、NOWAIT与1/5/10秒超时。原始SQL、历史账本、普通字段门禁及63项二开编号不变；完整上游集成基线仍为`a3eb7ef302961cba716dc78b39b93b60c467db0e`。
- 239只接受空推理倍率，在同一事务安装三项数据库检查约束保护新旧共存；原价字段可写，新增倍率在旧槽退役后才开放。240保留可空操作ID及部分唯一索引。切流/回滚验证约束真实定义；解除须重新核验唯一新槽、旧容器移除、机器、Compose、挂载及双入口。失败或回滚保留限制，不自动放宽门禁。
- 本地已通过专项Go单元/隔离集成、迁移239与联盟仓储回归、渠道/分组全推理级别空倍率计费、最终仓储全量集成、239成功/240失败保留保护、55项蓝绿状态机、7项证据测试、3项发布预检及最终golangci-lint（0问题）。北京时间09:56再次只读核验生产v0.1.245稳定blue、无pending、双Compose/挂载/资源参数与双入口正常；远端CI与生产发布结果以实际执行记录为准。
- 首轮新证据测试检出局部变量复用，已改正且同一断言重跑通过；并行临时挂载检查冲突仅影响本地测试环境，统一工作区POSIX测试路径后重跑，不改产品逻辑或断言。测试证明范围不含生产峰值、真实支付/提现和备份恢复；既有强制退役中断与用量未知风险不变。
- 操作说明见`docs/operations/2026-09-24-v0.1.246-release.md`及`docs/operations/blue-green-expand-migrations.md`；只读预检和测试日志保存在工作区`output/release-20260924-v0.1.246/`，不提交凭据或生产业务内容。
- 北京时间10:00只读迁移复检发现groups已有184/193鉴权缓存触发器。未打tag；审查后仅按完整定义、启用状态和函数SHA-256接纳该既有触发器，239仅改model_pricing会直接返回，未知触发器仍拒绝。新增真实193函数短路与定义漂移拒绝测试，不修改生产触发器或原SQL。

## 2026-09-24 v0.1.246发布验收（北京时间）

- 用户追加发布及专项迁移授权后的实际结果，不覆盖上述各阶段的历史边界。完整同步范围仍为`7c700729c23187d31ed320f6b19c790e2f194826..a3eb7ef302961cba716dc78b39b93b60c467db0e`，210项处置与63个二开编号不变；`LAST_FULLY_INTEGRATED_UPSTREAM_SHA`仍为`a3eb7ef302961cba716dc78b39b93b60c467db0e`，没有扩大上游范围。
- 专项最终候选`94dac3dcb880dcf96e3677827d3b4af7a55b9c98`通过候选门禁后快进main，主线门禁通过后创建新附注tag`v0.1.246`，随后标签门禁也全部成功。候选、main、tag各8项CI及2项安全扫描均未跳过，运行分别为`35945777793`/`35945777795`、`35946643480`/`35946643400`、`35947646975`/`35947646947`。发布tag固定于该提交，没有强推或移动历史标签。
- Release `35947647075`的10项作业全部成功，覆盖5个平台构建、6个发布附件、镜像、VERSION回写及实际蓝绿部署。VERSION回写提交`5afd0fa7a32b74cf5fdc829e17578741b11f5a62`仅改变版本文件；验收文档另走`codex/release-v0.1.246-evidence`分支门禁，不再次发布。
- 10:41:53切流至green/18082；11:43:01旧blue回收后恢复stable、pending=null，11:43:02三项倍率保护自动解除。11:44:11最终只读验收确认双入口HTTP/2 200、同一发布SHA与镜像、仅green且无回滚；生产Compose/config、资源、bind mount和PG/Redis容器及启动时间均与发布前一致，3600秒响应头与5秒usage超时保持。
