...
> **尚未生产部署。** server1 本轮只读 SSH 多次在 banner 握手阶段超时，无法重新核验线上版本及资源。本地/CI 的隔离测试数据不是生产升级耗时。
...
- 必须重新确认 server1 的线上版本、目标主机、实际挂载、双实例资源预算，以及首次迁移状态；不能用 2026-09-15 早前的报告替代本轮确认。
...
第一次迁移需要额外解决两件事：旧版无法证明排空；历史只读报告中的单应用池上限 512 / 数据库容量 600 与单应用约 80% 内存预算不支持原值直接双开。这些是待核验的历史值，不是本轮已确认生产值；资源变更不得由部署脚本自行执行。
