...
> `v0.1.237` 已于 2026-09-16 通过 tag/Actions 退役旧 v0.1.234 并复用 `blue/18080`；切流确认 1.179 秒。green v0.1.235 已按用户新授权于17:49强制回收，当前仅blue运行；下一候选固定为 `green/18082`，不新增端口。
> **2026-09-16策略变更：** tag发布改为观察一小时，超时以60秒关停收尾后强制退役；可能中断旧请求和用量收尾，不再承诺超时连接无损。下述历史“超时无限保留”仅描述旧策略，现行实现与本次生产取证见 `docs/operations/2026-09-16-hourly-forced-retirement.md`。
...
7. tag发布使用 `--window 3600 --force-after-window`；一小时内自然排空即正常回收，超时进入 `force_retiring`。切流起点和关停预算持久化，重跑不重新计时；未启用强制选项的手动执行仍在超时后保留旧槽。
8. 正常退役需核验旧worker退出及封闭凭据，使用 `docker stop -t -1`；明确启用的强制路径使用固定旧容器ID和 `docker stop -t 60`，保存中断及用量风险结果，停止后清理旧归属/socket。旧代理尚存时禁止复用槽位，不强杀其他站点连接。
