...
- 本地验证：Linux 后端全量单元测试、上游检查竞态测试、后端 lint、前端 lint/类型检查、41个关键测试文件共478项、Canvas 34项测试及两套前端生产构建通过；真实构建 HTML 注入回归和嵌入式后端构建通过。隔离浏览器完成11个截图场景，覆盖1440×1000桌面、390×844移动端、折叠、深浅主题、长版本、普通用户、网络失败、GitHub不可用与旧缓存；检查前后导航纵坐标均为96px。开发服务启动被本地环境策略拒绝，界面验证直接读取构建产物并使用浏览器内模拟接口，未访问生产。
