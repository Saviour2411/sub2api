...
| --- | --- | --- | --- | --- | --- |
| 2026-07-25 | `0.1.210` / 本次提交 | 修复 | `CUST-PROD-004` | 修复上游 `session_id` 与本地 `group_id` 合并后，`batch_image_jobs` 插入语句有 38 个目标列和参数但仅保留 37 个占位符，导致所有批量图片任务创建在 PostgreSQL 上失败；补齐 `$38` 并增加 SQLMock 列/参数绑定回归。 | repository 定向与全包单元测试、golangci-lint、后端构建；GitHub Actions 集成测试复核 |
