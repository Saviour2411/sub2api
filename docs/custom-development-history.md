...
- 当前生产核验（2026-09-24 11:44:11）：`0.1.246`、revision `94dac3dcb880dcf96e3677827d3b4af7a55b9c98`在green/18082接流，状态stable、pending=null；239/240已执行且并行期倍率保护在旧槽退役后解除，原配置、资源与数据挂载不变。旧blue一小时后强退，退出137且非OOM、用量收尾完整性未知，详情见`docs/operations/2026-09-24-v0.1.246-release.md`。
- 历史生产核验（2026-09-21 22:25:37）：`0.1.243`、revision `6154c64ffc83f180f7e8f118ccbd5182b8cb0f52` 在blue/18080接流，状态stable、pending=null；兼容字段迁移已完成，生产保护项不变。旧green按一小时策略强退，退出137且非OOM、用量收尾完整性未知，详情见 `docs/operations/2026-09-21-v0.1.243-release.md`。
- 历史生产核验（2026-09-16）：`0.1.237`、revision `004cc47a2090a35b7cc94a6463e97273b86562fb` 在 blue/18080 接收新流量；当时v0.1.234已正常退役，v0.1.235 green按真实工作自然排空。资源上限、PG/Redis和持久挂载保持不变。详情见 `docs/operations/2026-09-16-legacy-retire-gate.md`。
...
- 本地代码基线日期：2026-09-24（北京时间，上游同步与后续获批发布；生产事实以上述独立验收为准）
- 本地版本：`0.1.246`（本地同步阶段保持0.1.245；后续经追加授权发布，由Actions回写版本，不移动旧标签）
- 本地同步前代码基线提交：`5fc1a94713062e0d08fe82848a3a3975f82efa19`；本轮合并提交与验证结果见 `docs/upstream-sync-history.md` 的 2026-09-24 记录
- 已完整集成的上游提交：`a3eb7ef302961cba716dc78b39b93b60c467db0e`
- 本次同步范围：`7c700729c..a3eb7ef30`，210个上游提交（122个非合并、88个合并）；63项既有能力族编号全部保留，无跳过或延期提交；本地覆盖、实际验证和未覆盖范围见本轮同步记录
...
| --- | --- | --- | --- | --- |
| `CUST-GW-001` | 首 Token 超时与请求结果归因 | 在首个有效下游语义输出前执行可配置超时，metadata、preamble 和 keepalive 均不解除守卫；compact fallback 内部信号会撤销首次尝试暂存的响应，避免第一次失败提交空 HTTP 200，二次 fallback 失败返回真实错误；区分 pending/final outcome、部分响应、客户端断开和流内失败，避免错误重试或漏结算。WS 透传按轮等待合法终态，过早 close 不误判成功。HTTP client 从真正发起上游请求时开始计时，非流式 CC 不创建守卫。指定分组模式按 `CUST-GW-012` 忽略前导纯空白，保留原始空白直到有效输出一起交付；全局模式保持原有首输出语义。 本次上游适配：本次接入 pending-turn 与取消归因时仍保留逐轮语义输出、合法终态和一次结算规则。 2026-09-15 上游适配：接入原生 Codex Images 和 Gemini 带内错误登记时保留首语义输出、attempt 完结及部分响应归因。 2026-09-20 上游适配：共用 CC 出站增加 DeepSeek 历史推理占位时保留本地三返回值、firstTokenAttempt 生命周期及 Kimi 参数重试，不改变首输出计时或已提交响应禁止重放边界。 2026-09-24 上游适配：Gemini 传输错误切号前仍完结首 Token attempt；终态提前收尾、部分 usage 与已输出禁止重放规则保留。 | `backend/internal/service/first_token_timeout.go`、`backend/internal/service/openai_gateway_passthrough.go`、`backend/internal/handler/openai_upstream_outcome_tracker.go` | 生效中 |
...
| `CUST-GW-003` | 流式心跳与长请求保活 | 支持响应前心跳、普通 Responses 与 compact 的 SSE 心跳（含透传首输出前保活）和 Images JSON keepalive；首 Token 计时期间按路径暂停会提前提交状态的心跳，写出判定扣除非语义 keepalive 字节，客户端断开后可继续排空上游完成计费。 本次上游适配：HTTP/2 PING 保活纳入上游连接配置；断连排空仍与首输出守卫协同，不恢复已断开的下游写入。 2026-09-15 上游适配：WS 常驻读循环应答 ping；原生图片流仍按交付/断连状态处理，保活不代替语义输出。 2026-09-15 本地修复：Anthropic API Key 透传改为按最后一行上游数据的剩余空闲时间精确计时，消除180秒阈值可能拖到近360秒才检查的偏差；上游心跳/注释/空行继续续期，下游保活不续期，完整终止与断连排空语义不变。安全重试按 `CUST-GW-013` 独立启用；终止判定进一步要求完整事件分隔空行，事件名称行或截断终止不视为成功。 2026-09-20 上游适配：OpenAI HTTP/2 探测及应答期限恢复为15秒/15秒，其他长流参数保留；Gemini 原生流对 Go/Python GenAI 客户端不发送 SSE 注释，普通客户端保活及本地首输出/断连处理保留。 | `backend/internal/service/gateway_anthropic_passthrough.go`、`backend/internal/service/openai_compact_sse_keepalive.go`、`backend/internal/service/openai_gateway_passthrough.go`、`backend/internal/service/openai_images_json_keepalive.go`、`backend/internal/service/gateway_upstream_response.go` | 生效中 |
...
| `CUST-GW-007` | OpenAI 高级调度器 | 支持 sticky weighted、订阅优先、Top-K 和 priority/load/queue/error-rate/TTFT/reset/quota/session 等权重；另有优先最近重置策略。 2026-09-18 上游适配：接入 Codex 规范窗口配额读取、选号耗时及粘性命中计数修复，继续保留本地调度与故障恢复约束。 2026-09-24 上游适配：合入普通选号 previous_response 路由与决策标签补齐，本地高级调度、按请求兼容性筛选和释放规则保留。 | `backend/internal/service/openai_account_scheduler.go`、`backend/internal/service/setting_parse.go` | 生效中 |
| `CUST-GW-008` | 增强故障转移与响应结算 | 覆盖非 JSON 2xx、SSE `event:error`、流式/非流式 Responses `error`/`response.failed` 与传输错误、请求级容量错误、图片服务错误、流式部分结果和已提交响应；仅在安全重建上下文时允许 WS 后续轮次换号，复用错误体并避免重复写帧，客户端正常关闭不计为账号故障。WS 每轮重新取得请求模型和渠道映射、记录 payload hash 并独立结算 usage；失败停调度后阻断后续轮。OAuth transport 插件一旦报告 `RequestSent=true`，HTTP、Images、Live 及流式路径均禁止换号、同号重试、compact retry 或响应体读取错误重放；已收到合法终态时，迟到的 close error 不覆盖成功。 接入不可用 continuation、历史委派和心跳初始化恢复。图片仍强制将真实上游 URL 转为内联 data URL/b64；新可选回填开关默认关闭，关闭它不关闭既有 URL 隐藏。两条图片下载链均使用下载专属公网防护，强制内联链仍按原失败/交付规则处理，不改变业务 HTTP/私网 upstream 开关。WS Cyber 失败终态返回错误并保留部分 usage，不伪装成成功。 本次上游适配：WS 每轮白名单、请求定价、Cyber 和停调度校验继续生效；通过策略后只占槽一次。终态成功写入后才触发结算，断连排空同时保留部分 usage 和失败归因；Live 新增账号参数不放宽 RequestSent 禁重放。 2026-09-15 上游适配：新增原生 Codex Images 分支并保留 ImageDelivered、ClientDisconnect、部分 usage 与端点元数据；RequestSent 禁重放及普通图片 URL 内联防护不变。 2026-09-24 上游适配：合入流读取错误分类与脱敏，但继续发送单一 response.failed 合法终态；错误码保存在 response.error，部分用量及响应提交标记保留。 | `backend/internal/handler/failover_loop.go`、`backend/internal/service/upstream_outcome_error.go`、`backend/internal/service/openai_gateway_response_handling.go`、`backend/internal/service/openai_plugin_transport.go` | 生效中 |
...
| `CUST-PROTO-003` | Anthropic Claude Code 上游模拟 | 支持全局开关，将普通下游请求按 Claude Code 客户端规则清洗、补充和转发；账号测试与真实网关使用同一策略。 纳入可选 CLI 版本环境覆盖；强制模拟后的最终出站 UA 与 billing 指纹版本一致，并保留 Unicode 字符索引算法。 2026-09-15 上游适配：Claude 模拟、网关与账号测试继续复用本地清洗策略；本地 system 配置与上游缓存块上限并存。 2026-09-24 上游适配：接入 Claude Code 动态版本；messages、count_tokens 和账号测试共用单次请求的 UA/billing 版本快照，运行期切换不能造成头体不一致，全局强制模拟继续覆盖 API Key。 | `backend/internal/service/gateway_anthropic_mimicry_sanitize.go`、`backend/internal/service/gateway_anthropic_oauth_mimicry.go` | 生效中 |
...
| 编号 | 功能 | 当前行为与边界 | 关键入口 | 状态 |
| --- | --- | --- | --- | --- |
| `CUST-ACC-001` | 账号策略默认值与模型白名单 | 创建账号时提供本地 pool mode 默认策略、模型白名单和平台特定控件；API Key 账号落库前可按当前代理、并发、端点和模型映射预览上游模型，创建后再正式同步能力元数据；批量编辑保持这些字段的 API 契约。分组 Fast 与 Free Fast 策略贯穿创建、更新、复制、管理 API 和认证快照，不改变既有账号默认策略。 新上游请求标识头、图片回填开关与本地失败策略、图片尺寸字段组合保存；模型同步成功提示继续显示数量。认证快照升至 v24，同时保留固定账号清单、长上下文与 Free Fast 字段。 本次上游适配：本次将旧 models_list_config 数据迁移为 model_allowlist；获批后同时约束模型展示与实际请求准入，未启用时不限制。认证快照升级到 v24，固定账号清单、长上下文和 Free Fast 字段完整保留。 2026-09-18 上游适配：按批准方案让 DeepSeek 无显式映射时使用上游模型名单，不再任意兜底；显式映射、本地其他 Provider 白名单及默认策略保留。 2026-09-20 上游适配：账号端点能力增加默认不启用的 Seedance，继续保留本地图片尺寸类型、模型白名单、pool mode 和表单策略。 | `frontend/src/components/account/poolModeDefaults.ts`、`frontend/src/components/account/ModelWhitelistSelector.vue`、`frontend/src/components/account/CreateAccountModal.vue`、`backend/migrations/145_group_models_list_config.sql` | 生效中 |
...
`CUST-BILL-005` 导出边界（2026-09-24）：使用记录导出复用原筛选和用户归属校验，但顺序固定为不可变 ID 倒序，不跟随表格临时排序。游标为十进制字符串，避免大 ID 的 JavaScript 精度丢失；新写入的更大 ID 不插入当前遍历，导出不是跨批次数据库事务快照。单批查询20秒超时，批次间隔1250毫秒；429按 Retry-After 等待，网络和部分5xx最多重试5次，不放宽现有限流。用户CSV只累计编码后的分片；管理员XLSX仍受浏览器内存及Excel单表行数限制，未引入持久化后台导出任务。取消或离开页面立即终止后续请求，取消不再提示失败。不改导出费用字段和计费规则，不需要数据库迁移。

### 渠道监控与可观测性

| 编号 | 功能 | 当前行为与边界 | 关键入口 | 状态 |
| --- | --- | --- | --- | --- |
| `CUST-OBS-001` | 流式渠道监控与随机抖动 | `probe`/`quota_probe` 渠道监控和模板可以选择流式检测，`quota` 模式独立读取额度；探针总超时与响应头超时统一为 600 秒，Gemini 流式与非流式响应均遍历首个候选的全部文本分段并跳过思考分段；检测间隔支持正负随机抖动，避免大量渠道同一时刻探测，并保留 provider/endpoint/请求体的本地扩展。渠道分组按用户访问范围过滤，OpenAI 可见 TTFT 的统计口径由管理员设置统一控制。 2026-09-18 上游适配：吸收监控自动刷新间隔修复；Provider 测试继续按 PROVIDERS.length 验证并保留 MiniMax，避免固定数量断言遗漏本地供应商。 2026-09-24 修复启动/待激活/退役及共享执行资格拒绝后未释放 in-flight 导致永久停探；无样本可用率为null，真实失败仍为0%；列表和详情区分过期数据，显示样本数及最后检测时间，隐藏过期延迟/额度并同步刷新多窗口缓存。过期阈值包含两倍最大抖动间隔及完整探测预算。 | `backend/internal/service/channel_monitor_runner.go`、`backend/internal/service/channel_monitor_aggregator.go`、`backend/internal/service/channel_monitor_checker.go`、`backend/internal/service/channel_monitor_const.go`、`backend/ent/schema/channel_monitor.go`、`backend/migrations/141_channel_monitor_stream_enabled.sql`、`backend/migrations/226_channel_monitor_quota_mode.sql` | 生效中 |
...
| `CUST-PROD-002` | 本地模型市场（`/models`） | 按指定分组公开模型、协议格式、价格和介绍，管理端可配置开关、介绍和分组范围。默认启用，但只返回公开白名单字段；与上游默认关闭的 `/model-plaza` 及面向 Codex 客户端的分组模型清单独立共存，二者均不替代本地入口。 本次上游适配：公开模型列表使用同组 model_allowlist，支持精确/通配筛选，启用后无匹配或遗留空配置不回填禁止模型；原公开分组范围与默认开关保留。 2026-09-24 上游适配：市场展示接入通用倍率并克隆配置映射；Fable 5.1继续无默认三倍，显式倍率仍可展示与配置。 | `backend/internal/handler/model_marketplace_handler.go`、`backend/internal/handler/openai_codex_models_handler.go`、`frontend/src/views/ModelMarketplaceView.vue` | 生效中 |
...
| `CUST-PROD-008` | 用户定制公开余额查询 | 管理员按用户生成唯一长期查询链接 `/balance-query#随机码`，默认不生成；32字节安全随机数，仅存摘要及加密原文，可复制、停用、启用、重置，重置立即废止旧链接，独立于授信开关。页面不恢复登录态，兼容后台模式，仅公开用户名（空时用ID）、ID、余额、安全充值历史和全局有序纯文本充值方式（最多20项、键50字、值500字）；记录默认20条、最多50条分页，仅已到账余额兑换/充值、管理员正向加款和临时授信，普通来源固定备注，授信显示实际入账金额。接口不接受用户ID或URL令牌，仅专用头传码；IP每分钟30次、有效链接60次，429附Retry-After，Redis故障503。页面和接口no-store/noindex/no-referrer，无登录凭据、自动轮询及账户写入，停用/删除用户与失效链接统一不可用。2026-09-20 后台用户定制投影新增邮箱，供管理员搜索和识别；公开 DTO 和余额页仍不包含邮箱，用户名为空仍仅以 ID 兜底，不修改用户资料、余额、授信或链接规则。 | `backend/internal/handler/balance_query_handler.go`、`backend/internal/service/user_customization.go`、`frontend/src/views/public/BalanceQueryView.vue`、`frontend/src/components/admin/user/UserCustomizationsPanel.vue` | 生效中 |

### 风控与内容审核

| 编号 | 功能 | 当前行为与边界 | 关键入口 | 状态 |
| --- | --- | --- | --- | --- |
| `CUST-RISK-001` | 本地人工审核对话 | 内容审核可进入本地人工审计，持久化请求/响应记录；管理端支持列表、详情、下载和删除。统一安全审计协调器接入上游 prompt audit，协调器未配置时继续使用本地内容审核，审计服务过载时按配置执行回退。 2026-09-20 上游适配：OpenAI/TypeSafe 独立引擎配置与全部 LocalAudit 字段共存；保存、切换引擎、快照克隆和前端草稿保留审计开关、场景、容量、图片排除、客户端/工具规则及捕获并发。默认引擎仍为 OpenAI，日志新增可空 engine_meta。 | `backend/internal/securityaudit/coordinator.go`、`backend/internal/service/content_moderation_local_audit.go`、`backend/internal/handler/security_audit_helper.go` | 生效中 |
| `CUST-RISK-002` | Cyber 会话阻断 | `cyber_policy` 命中可沿网关、审计和计费链路透传，并按配置对会话做 TTL 阻断；Responses `response.failed` 先解析 usage 再标记会话，WSv2 透传会在 `AfterTurn` 前设置标记，避免错误终态漏记 Token，用量记录允许 `cyber_blocked` 类型。事件只有在全局风险控制、内容审核、非 `off` 模式以及 group/model scope 均允许时才记录；运行时快照读取失败时安全跳过事后副作用，不扩大阻断范围。 WS 各路径标记与同一逻辑轮次的失败转移链去重，两个原子状态分别保存已阻断与失败转移待判定；不扩大风控 scope，保留失败终态的部分 Token。 2026-09-15 上游适配：WS 作用域与抢占更新后仍逐轮执行 Cyber 策略、失败去重与部分 usage 记录，不扩大作用域。 2026-09-20 上游适配：保留 cyber_policy_exclude_from_ban_count、作用域校验和事件副作用；该配置不随审核引擎切换重置，既有 Cyber 日志不伪造第三方审核引擎来源。 | `backend/internal/service/content_moderation.go`、`backend/internal/service/openai_ws_v2_passthrough_adapter.go`、`backend/internal/handler/security_audit_helper.go`、`backend/migrations/174_allow_cyber_blocked_usage_request_type.sql` | 生效中 |
...
| 编号 | 功能 | 当前行为与边界 | 关键入口 | 状态 |
| --- | --- | --- | --- | --- |
| `CUST-UI-001` | 独立二开管理页 | 将上游同步监控、模型广场、每日签到、网关运行策略和用户定制集中在 `/admin/custom-features`，与上游通用设置隔离；页面使用宽屏管理台布局展示高密度数据；上游管理始终排在第一个并默认选中，其余页签保持相对顺序。用户定制独立页签支持搜索用户、余额展示、全局充值方式增删排序、专属查询链接管理和授信配置；重置链接与恢复授信均需确认，恢复窗口显示用户、阈值和单次金额，沿用管理员鉴权、写幂等及敏感读取审计。2026-09-20 搜索统一为用户名和邮箱不区分大小写的包含匹配、ID 精确匹配，去除首尾空白，提交时回到第一页，列表和总数共用过滤条件并排除软删除用户；不搜索备注或 API Key。列表、编辑和确认窗口优先显示用户名，空时显示邮箱，始终保留 ID；有用户名时附带邮箱。 | `frontend/src/views/admin/CustomFeaturesView.vue`、`frontend/src/components/admin/user/UserCustomizationsPanel.vue`、`backend/internal/server/routes/admin_custom_features.go`、`backend/internal/repository/user_customization_repo.go` | 生效中 |
...
| `CUST-UI-004` | 表格和导航稳定性 | 保留账号列表禁用虚拟化、查询后滚动重置、分页大小持久化、侧边栏滚动位置，以及日期和列设置下拉浮层不被表格 sticky 表头覆盖；用量窗口统一展示，隐藏的缓存 tooltip 不再撑出横向滚动，账号到期时间按本地时区严格解析。 账号列表接入 lite DTO 和编辑时详情回取；本地禁用虚拟化、分页/滚动和列设置 Teleport 浮层保留。请求 ID/上游 ID 默认隐藏，测试卸载浮层实例避免跨用例串扰。 本次上游适配：账号操作菜单、用量密钥分页和清单编辑即时更新纳入，保留 DataTable 非虚拟化及本地浮层与列配置。 2026-09-15 上游适配：账号创建复用本地 upstreamModelSyncPreviewRequest；OpenCode adaptive 协议进入同一预览路径，模型白名单测试对齐本地接口并保留原断言。 2026-09-18 上游适配：API Key 批量编辑与本地列设置入口同时保留，不以新增批量弹窗替换列配置。 2026-09-24 上游适配：日期弹层保留 Teleport、固定定位及窄屏边界，同时接入关闭放弃草稿、应用后保留和跨午夜刷新。 | `frontend/src/components/common/DataTable.vue`、`frontend/src/components/common/ColumnSettingsDropdown.vue`、`frontend/src/components/layout/TablePageLayout.vue`、`frontend/src/components/layout/AppSidebar.vue` | 生效中 |
...
| 编号 | 功能 | 当前行为与边界 | 关键入口 | 状态 |
| --- | --- | --- | --- | --- |
| `CUST-OPS-007` | 蓝绿兼容字段迁移门禁 | 普通规则仅允许绑定旧提交、SQL和旧代码摘要的新增可空字段，238b已随v0.1.243发布。2026-09-24经用户追加批准，239/240采用固定SQL摘要专项，不放宽普通门禁、不改上游SQL。专项限PostgreSQL18、无继承且至多16MiB的普通表；未知用户触发器拒绝，仅按定义及函数摘要接纳184/193既有groups鉴权缓存触发器。NOWAIT表锁及1秒锁等待/5秒语句/10秒事务预算；失败不自动重启候选或切流。239仅允许无显式倍率，事务内安装检查约束，临时禁止新旧实例共存期写入推理倍率，普通价格可写；切流/回滚前核对保护，旧槽退役并重新安全核验后自动解除，回滚或失败保留。240核验可空操作ID及部分唯一索引。已提交迁移不反向删除，不承诺零延迟或无损强制退役。239/240已随v0.1.246发布，旧槽退役后保护已解除，验收见本轮发布记录。 | `deploy/blue-green/evidence.py`、`deploy/blue-green/expand-approvals.json`、`deploy/blue-green/pricing-guard.py`、`backend/internal/repository/migrations_blue_green_special.go`、`docs/operations/blue-green-expand-migrations.md` | 发布门禁 |
...
| `CUST-OPS-004` | 本地 CI 与安全门禁 | Push/PR 运行 Go 1.27.0 单元和集成测试、Node 20 前端及 Canvas 测试、golangci-lint 2.13、部署脚本检查及依赖安全扫描；gosec G703/G704 不再全局排除，生产代码逐点说明，测试文件按明确路径豁免。发布前以远端 CI 为最终门禁；本地工具版本低于目标 Go 版本时必须如实标记静态检查未验证。 新增12项隔离部署脚本回归并接入macOS shell job，覆盖禁止清单、不一致清单、命名卷/错误目录、重建前健康失败及重建后健康失败；无检查工具不能伪报成功。 本次上游适配：新增上游 i18n 完整性门禁与本地 featureFlags 门禁并存。ZIP 安装前关闭与本地幂等清理共存；PG 备份迁移锁以本地 mock 验证，未执行真实备份/恢复。用户定制管理、公开余额查询客户端与页面、使用记录表格及输出速率的5个前端测试文件纳入固定CI关键测试清单。Claude 透传流安全重试与精确空闲计时纳入 Linux 定向竞态检查，配置 API、流诊断与错误详情组件纳入关键前端测试；PR #22 安全扫描发现的 gRPC 漏洞通过最小修复版本升级解决，不增加豁免。 2026-09-18 上游适配：新增 API Key 批量编辑关键测试清单与既有门禁并存；保留本地 gRPC 1.83.2 安全版本、validator 直接依赖及 Wire/Ent 工具依赖，不增加安全豁免。 2026-09-20 上游适配：发布辅助脚本14项测试与原有流竞态、生命周期、蓝绿门禁同时保留；新增审核引擎与本地字段的交叉回归、响应别名与用量元数据断言，未删除断言或增加安全豁免。 2026-09-24 上游适配：上游新增用例与本地交叉回归同时保留；动态UA、旧版备份锁、超时存活任务、终态错误、定价白名单、日期与CSV验证纳入本轮记录。 | `Makefile`、`.github/workflows/backend-ci.yml`、`.github/workflows/security-scan.yml`、`backend/.golangci.yml` | 运维约束 |
...
| `CUST-OPS-006` | 禁止网页自更新 | 禁止网页更新、下载、回滚、重启和自动同步代码。已移除的 `/api/v1/admin/system/` 下 `check-updates`、`rollback-versions`、`update`、`rollback`、`restart` 路由继续返回 404。2026-09-23 允许新增管理员只读 `upstream-version` 查询及发布页提示；`version` 仅返回当前构建和已同步来源，不访问 GitHub。新检查服务只获得最新 Release 查询能力，不恢复旧更新服务、操作锁或执行入口。共享 GitHub 客户端和首次安装使用的重启工具保留；实际升级仍走定制版标签发布和蓝绿部署流程。 | `backend/internal/handler/admin/system_handler.go`、`backend/internal/server/routes/admin.go`、`backend/internal/service/upstream_version.go`、`backend/internal/service/github_release.go` | 运维约束 |
...
- 关联`CUST-OPS-001`、`CUST-OPS-003`、`CUST-OPS-007`，不新增或退役二开编号，63项能力族全部保留。实现提交`94dac3dcb880dcf96e3677827d3b4af7a55b9c98`通过候选、主线、标签各8项CI及2项安全扫描；新附注tag`v0.1.246`与Release `35947647075`的10项作业均成功，没有重打标签或绕过门禁。
- 10:41:53切流green/18082，11:43:01旧blue回收后恢复stable、pending=null，11:43:02自动解除倍率保护；11:44:11只读核验发布版本/摘要、唯一活动槽、双入口和迁移结构均符合预期。309条历史迁移及摘要未变，仅新增239/240，三项临时保护约束已消失；普通字段门禁及原SQL不变。
- 原Compose/config哈希、批准资源参数、HTTP upstream、bind mount、PG/Redis身份及启动时间均不变，保留3600秒响应头等待、5秒usage任务与按用户串行扣费；green切流后未重启。VERSION仅由Actions回写至0.1.246，生产验收记录不另触发部署。
- 旧blue一小时到期时仍有HTTP 5、SSE 3、会话租约832，按既有策略强制退出137、非OOM，clean_exit=false、usage_loss_unknown=true；817条旧归属已清理。7012次健康探针无异常不能证明旧请求无中断或用量完整，不改账单和退役策略。
- 真实付费Provider、支付/回调/提现、完整账单、备份恢复演练、全二开浏览器端到端及峰值压测仍未验证；既有内存预算OOM风险保留。完整时序、CI、镜像与迁移证据见`docs/operations/2026-09-24-v0.1.246-release.md`；本记录通过独立文档分支门禁后再合入主线。
...
- 同日10:00生产只读检查识别184/193已有groups触发器；按真实定义和函数摘要接纳其模型定价更新直接返回路径，新增真实193函数及定义漂移拒绝测试。其他用户触发器不放行，不修改线上触发器。

### 2026-09-24 上游0.2.8本地增量同步

- 完整合并固定范围 `7c700729c..a3eb7ef30`。38个文本冲突按批准方案逐块组合，未整文件采用任一侧。保留本地0.1.245版本线、63项编号、禁止网页更新及所有生产保护边界。
- 通用思考倍率与本地请求模型/严格定价组合，保持Fable未配置为1倍；新GPT-6 Sol/Luna接入本地严格规范化，不放宽未知模型识别。流错误保留response.failed终态，同时接入分类、脱敏及单次输出标记。
- 月度归档接入本地实例归属及优雅排空。数据库写锁保留旧键并拒绝错误后改用Redis；恢复开始与结果写入保留owner，只在明确死亡时回收，归档保护和持久写入失败不删对象规则保留。
- 动态Claude版本保持全局API Key强制模拟；messages、count_tokens及账号测试的UA与billing版本来自同一快照。日期弹层继续Teleport，CSV继续游标导出、节流、取消与公式保护。
- 补充旧锁键/共享数据库/异常不降级、存活未知owner超时保护、恢复归属、动态版本翻转及新模型白名单回归；适配上游日期测试的真实Teleport定位与通用倍率表单，不删除断言。前后实际命令、首次失败与修复复跑、未验证项集中记录在本轮同步历史。
- 新迁移239/240只进行隔离验证，均不符合现行蓝绿迁移门禁；未扩展兼容白名单。没有push、PR、标签、SSH或生产操作，也不将本地测试等同于真实Provider/生产验收。

### 2026-09-23 恢复版本来源并新增只读上游提示

- 涉及 `CUST-OPS-006`、新增 `CUST-OPS-008`。修复删除旧更新服务后遗漏 `SettingService.SetVersion`，导致首屏注入版本为空且前端永久缓存空值的问题；不在前端写死当前版本，不主动提升发布版本。
- 嵌入已核验的完整同步基线：`Wei-Shaw/sub2api`、`0.2.7`、`7c700729c23187d31ed320f6b19c790e2f194826`、`2026-09-20T23:09:09+08:00`。这是已有同步事实的结构化记录，不代表本次合入新上游代码。
- 管理员的两个版本接口均须通过管理员鉴权，公共配置不暴露同步来源；同步来源和当前版本不等待 GitHub。最新版本仅与已同步原版基线做语义化比较，排除草稿、预发布及无效标签；发布链接限定原仓库，不暴露 GitHub 凭据或错误详情。
- 界面保留品牌样式，管理员固定三行信息，折叠时可打开详情；检查失败不弹全局错误，不影响导航，不恢复任何更新执行能力。
- 轮询补充浏览器与服务器时钟偏差保护：服务端返回成功缓存但本地计算已到期时，恢复正常30分钟间隔，避免每秒重复请求；包含对应前端回归。
- 回归覆盖版本注入及接口一致性、真实普通用户权限、已删除路由404、缓存/超时/退避/并发/取消与旧结果保留、前端身份差异和计时器清理。关键前端用例、上游检查竞态、实际构建 HTML 注入检查及同步元数据校验纳入 CI；元数据校验要求提交是当前历史祖先且其上游 `VERSION` 与记录相同。
- 本地验证：Linux 后端全量单元/集成测试、上游检查竞态测试、后端 lint、前端 lint/类型检查、41个关键测试文件共479项、Canvas 34项测试及两套前端生产构建通过；真实构建 HTML 注入回归和嵌入式后端构建通过。隔离浏览器完成11个截图场景，覆盖1440×1000桌面、390×844移动端、折叠、深浅主题、长版本、普通用户、网络失败、GitHub不可用与旧缓存；检查前后导航纵坐标均为96px。开发服务启动被本地环境策略拒绝，界面验证直接读取构建产物并使用浏览器内模拟接口，未访问生产。
- 交付边界：经 `codex/` 分支及 PR 验证后合入主线，等待主线 CI 和 Security Scan；无需数据库迁移，不打标签、不部署、不修改生产配置。远端验收结果随交付报告提供，本条不宣称已发布。

### 2026-09-21 v0.1.243 发布验收

- 关联`CUST-OPS-001`、`CUST-OPS-003`、`CUST-OPS-007`。实现提交`6154c64ffc83f180f7e8f118ccbd5182b8cb0f52`通过候选CI及安全扫描后快进主线，再推送新附注标签`v0.1.243`；候选、主线、标签各8项CI和2项安全扫描全部成功。Release `35604350151`的10项作业及实际蓝绿部署步骤全部成功，没有覆盖旧标签。
- 新增`engine_meta`实测为可空JSONB、无默认值，历史迁移账本摘要全部不变且只新增238b；实际活动迁移策略与Actions兼容证据一致。21:23:32切流blue/18080，双入口确认约1.156秒，22:24:41恢复stable、pending=null。
- 旧green按既有一小时策略强退，关停凭据记录HTTP 10、SSE 4、会话租约495，退出137且非OOM、clean_exit=false、usage_loss_unknown=true。7016次健康探针无异常不代表旧请求无中断或账单完整，字段兼容审批不扩大为无损退役承诺。
- 最终只读验收确认新blue未重启，PG/Redis身份、启动时间、重启计数、资源参数、原bind mount、双Compose和持久配置均不变；保留3600秒响应头超时及5秒usage任务超时。完整镜像、归档摘要、时序和未验证边界见`docs/operations/2026-09-21-v0.1.243-release.md`。
...
| --- | --- | --- | --- | --- | --- |
| 2026-09-24 | 本次修复提交 / 不提升版本 | 修复 | `CUST-OBS-001`、`CUST-BILL-005` | 修复渠道监控在生命周期拒绝执行后永久保留本地in-flight标记的问题，避免无样本被误显示为0%或陈旧状态继续显示正常；修复用户导出第61页触发限流、管理员深分页重复COUNT/OFFSET造成长时间导出失败。新导出接口保留原鉴权、权限过滤及DTO投影，前端按稳定游标限速读取、重试和取消。只提交代码并验证CI，不打标签、不部署或重启生产，不修改历史监控与用量数据。 | Go全量unit与真实PostgreSQL/Redis integration、渠道调度定向race、golangci-lint（0 issues）、CGO关闭构建及实际前端嵌入版本回归通过；前端329文件/2511用例、ESLint、类型检查、生产构建，画布8文件/34用例、类型检查和构建通过。回归覆盖生命周期拒绝后恢复、无样本/真实0%/过期预算、游标筛选/用户隔离/并发插入、61页节流/429重试/取消。Chrome本地构建与模拟API完成1440px/390px、深浅主题、渠道详情、CSV/XLSX下载及行列校验、进度与取消验收；未做生产压测。远端CI与Security Scan以最终提交的工作流结果为准。 |
| 2026-09-22 | `0.1.244` / 发布候选 | 新增与修复 | `CUST-GW-015` | 为 Kimi 参数兼容重试增加非法思考类型独立开关，明确400且出站值非法时删除整个 thinking 重试一次；默认关闭，新增配置读写/缓存、管理页中英文文案与旧客户端部分更新保护，累计规则上限由四次扩为五次。按用户追加授权，CI 与安全扫描通过后推送新标签并自动蓝绿部署；不修改生产开关及退役策略。 | 修复前真实上游三组 HTTP/2 流式对照已复现400，剥离/auto均完整成功。后端 service、admin handler 全量 unit 与 Kimi/审计定向竞态测试通过；前端326个文件、2481项测试全部通过，类型检查、改动文件 ESLint、gofmt 与 diff 检查通过。首次前端全量仅既有 browserStorage 测试失败，禁用 Node 25 原生 Web Storage 后定向及全量重跑通过，未修改无关代码。首轮远端 CI 检出新增分支的 staticcheck QF1003，改为等价 switch 后定向单测和与 CI 同版 golangci-lint 2.13.0 全量检查通过（0 issues）。新增配置、匹配边界、流式/非流式恢复、限次、换号保留与界面回归均通过；本行记录发布前验证，不代表线上部署已完成。 |
...
7. release、Compose、镜像或网页版本管理变化：`CUST-OPS-001` 至 `CUST-OPS-006`；尤其不得恢复网页自更新、回滚或重启入口。
...
- 再次追加发布授权：主线 `eed208250` 的7项CI和2项安全扫描均通过后，用户要求更新tag并通过GitHub Actions自动蓝绿部署。已创建固定指向该提交的附注标签 `v0.1.241`，不改生产开关、资源、挂载或既有一小时退役策略；发布结果与实际验收见 `docs/operations/2026-09-19-v0.1.241-release.md`。
- 发布验收：tag的7项CI、2项安全扫描及Release `35431430291` 的5项任务均成功；16:26:17切流blue/18080，17:27:27旧green回收后恢复stable、pending=null。新实例、数据库、Redis健康，资源、挂载、双Compose及持久化配置哈希保持不变。
- 旧实例为到期强制退役，关停前HTTP19、SSE12、会话租约929，退出137且非OOM；clean_exit=false、usage_loss_unknown=true。7016次健康探针无异常不等于旧请求无中断或账单完整，未改退役策略或补写账单。
- 发布过程未自动开启开关；16:37:56管理端审计129155确认admin/JWT保存将安全重试和提前保活同时设为true，最终保留该管理操作，不自行回退。代码默认关闭及原有首字统计仍保持不变。

### 2026-09-20 删除网页自更新并修复用户定制搜索（北京时间）

- 实现阶段基线 `71213b65c123c6c553e7f3cd8261be1270ea8757`，版本保持 `0.1.241`。新增 `CUST-OPS-006`，修改 `CUST-UI-001`、`CUST-PROD-008`。实现阶段仅本地修改与验证，不提交、推送、打标签或部署，未连接生产服务器，无数据库迁移。
- 网页版本区不再调用上游版本检查、更新、回滚或重启接口；服务端移除对应五条路由及专用更新依赖，仅返回当前构建版本。共享 Codex 客户端版本同步、首次安装重启、定制版标签发布和蓝绿部署均保留。
- 搜索按用户名和邮箱不区分大小写包含匹配，ID 精确匹配，保留空白清理、提交查询、重置页码及软删除过滤。邮箱仅增加在管理员响应和身份显示中，公开余额 DTO、页面、账户数据和授信规则不变。
- 前端全量 324 个文件 / 2441 个用例、类型检查、非改写 ESLint、生产构建通过；构建仍有已有 Browserslist 数据陈旧、动态/静态混合导入和分包体积提示，不更新依赖。新增版本、后台 API、搜索和确认窗口测试加入关键前端回归清单。浏览器复验发现无空格长邮箱会溢出确认消息，已补充自动换行，确认与取消事件保持不变。
- 后端 Go 1.27 全量 unit（58 个有测试的包）、真实 PostgreSQL 18.1 / Redis 8.4 的 repository 集成包、golangci-lint 2.13（0 issues）及带前端嵌入的服务端构建通过。搜索覆盖大小写、首尾空格、中文、邮箱片段、精确 ID、空查询、无结果、软删除、备注排除和分页总数；授信并发、恢复代次、链接失效与公开字段隔离回归保留。新增路由测试验证五个旧接口 404、只读版本响应正常且仍需管理员鉴权；共享 Codex 版本同步及首次安装相关既有测试通过。
- 初次全量回归发现测试库其他用例也含 `box`，搜索用例改为核验相对既有结果的增量并明确排除软删除及备注匹配。WSL 的 Windows 挂载目录不支持 Unix socket 和 POSIX 文件权限，导致既有生命周期及首输出暂存测试失败；仅将测试临时目录切到工作目录内的 Linux tmpfs 后全量通过，随后已卸载，不修改这些业务逻辑。WSL 构建的 Git 所有权检查通过本次命令的 `-buildvcs=false` 避开，不改仓库信任配置或发布脚本。
- Playwright 以本地构建产物和隔离接口夹具检查 1440×1000、390×844：版本区只读且无上游检查请求；`box`、`BOX`、首尾空格及 ID 搜索、中文身份、分页后重新搜索回到第一页均通过；编辑、恢复和重置窗口显示正确身份及金额，长邮箱不溢出；公开页仅保留 ID、用户名和余额，不含邮箱，请求不携带登录凭据。截图位于 `output/playwright/update-search-*.png`，辅助夹具与日志位于 `tmp/web-update-search-20260920/`。
- 实现阶段验证边界：本地 Vite 后台启动被工具策略拒绝，未换壳重试，浏览器通过测试会话加载已构建的静态产物完成检查；当时未执行真实生产流量、实际首次安装重启或发布后验收。生产应用、数据库、Redis、配置、数据挂载和版本均未被该实现阶段修改。
- 后续发布授权：用户追加要求提交并推送代码、合入主线，待该固定提交的全部 CI 和安全扫描成功后，再创建新标签并通过 GitHub Actions 自动蓝绿部署。预计使用尚未占用的 `v0.1.242`，不覆盖旧标签，不修改生产资源、配置、开关或既有一小时观察及到期强制退役策略；本条仅记录授权与待执行流程，不代表发布已经完成，实际结果另行归档。
- 发布结果：功能提交 `ee1c56f75a38c8678ed338a4a1fecad51b08593c` 已推送主线，在7项CI和2项安全扫描通过后创建附注标签 `v0.1.242`。标签的7项CI、2项安全扫描及 Release `35462200005` 的5项任务全部成功；Actions 自动回写版本为 `0.1.242`，未移动旧标签。完整记录见 `docs/operations/2026-09-20-v0.1.242-release.md`。
- 生产验收：03:00:22 切流到 green/18082，双入口确认约1.176秒；04:01:31 旧 blue 回收后恢复 stable、pending=null。实际管理员只读请求中 `box`、`BOX`、` box `、`1054` 均匹配目标用户，五个已移除接口均404，公共设置及只读版本接口返回0.1.242；邮箱仅出现在管理员响应中，未回传生产凭据、明文邮箱或余额。
- 数据保护：切流后、观察中及退役后比对均确认数据库和Redis的容器身份、启动时间、挂载、资源以及双Compose、持久化配置、环境文件哈希未变，安全重试与提前保活继续保留管理员设置的true/true。新green的实例身份和启动时间在观察期间未变。
- 退役边界：旧blue按一小时策略强制退役，关停凭据记录HTTP17、SSE10、会话租约720，退出137且非OOM，clean_exit=false、usage_loss_unknown=true。7016次健康探针无异常不代表旧请求无中断或用量完整；本次未改退役策略、补写账单或重启数据库/Redis。
