export default {
  ...
  affiliate: {
    ...
    tips: {
      ...
      line2: '被邀请用户充值余额后，你可按其实际到账的余额额度获得 {rate} 的返利额度。',
      line3: '返利额度可随时转入账户余额。',
  ...
  admin: {
    ...
    channels: {
      ...
      form: {
        ...
        defaultPricing: '未命中默认计费',
        defaultPricingHint: '当模型专属定价和全局默认定价都未命中时，使用这里的 Token 单价计费。',
        defaultPricingRequired: '开启未命中默认计费时，至少需要填写一个默认价格',
        selectedCount: '已选 {count} 个',
        searchGroups: '搜索分组...',
    ...
    riskControl: {
      ...
      localAuditEnabled: '保存成功对话用于人工审计',
      localAuditEnabledHint: '开启后仅保存成功转发的对话过程到本地文件，不保存失败请求；文件会自动脱敏敏感字段。',
      localAuditMaxStorage: '本地保留上限',
      localAuditMaxCaptureConcurrency: '审计捕获并发上限',
      localAuditMaxCaptureConcurrencyHint: '达到上限时会临时跳过保存用户数据，请求仍正常转发；设置 0 表示不限制，不建议生产环境使用。',
      localAuditScenePolicy: '保存场景',
      localAuditSceneAll: '全部成功对话',
      localAuditSceneAllDesc: '保存所有成功转发的对话，适合临时扩大抽查范围。',
      localAuditSceneProgrammingOnly: '仅编程场景',
      localAuditSceneProgrammingOnlyDesc: '仅当命中客户端、工具调用或文件读写等编程信号时保存。',
      localAuditExcludeImage: '排除生图任务',
      localAuditExcludeImageHint: '开启后跳过 Images 生成/编辑请求，减少不必要的审计文件。',
      localAuditClientPatterns: '编程客户端匹配规则',
      localAuditToolPatterns: '编程工具匹配规则',
      localAuditPatternsHint: '每行一个关键词，大小写不敏感；用于匹配 User-Agent 或 Originator。',
      localAuditToolPatternsHint: '每行一个关键词，大小写不敏感；用于匹配请求或响应中的 tool name。',
      localAuditStoragePath: '保存目录',
      localAuditStorageStats: '当前 {records} 条，占用 {bytes}，上限 {max} GB；超出后自动清理最旧记录。',
      localAuditStabilityTitle: '稳定性兜底',
      localAuditStabilityHint: '当前捕获 {active} 个，已因过载跳过 {skipped} 次。估算内存按 并发上限 {concurrency} x 单请求捕获 {limit} x 4~5，再叠加服务基础内存和 Go GC 余量；当前服务器建议先使用 128。',
      localAuditRecords: '人工审计记录',
      localAuditRecordsHint: '本地保存的成功对话过程，供管理员人工复核。',
      localAuditLoadFailed: '加载人工审计记录失败',
      localAuditDetailFailed: '加载人工审计详情失败',
      localAuditDownloadFailed: '下载人工审计记录失败',
      localAuditDeleteFailed: '删除人工审计记录失败',
      localAuditDeleted: '人工审计记录已删除',
      localAuditDeleteConfirm: '确定要删除这条人工审计记录吗？此操作不可撤销。',
      localAuditEmpty: '暂无人工审计记录',
      localAuditCounts: '消息 {messages} / 工具 {tools} / 调用 {calls} / 结果 {results}',
      localAuditDetailTitle: '人工审计详情',
      localAuditDetailContent: '对话过程 JSON',
      download: '下载',
    ...
    accounts: {
      ...
      currentApiKeyHint: '这里显示已保存的密钥；下方输入框只用于替换，留空不会修改。',
      newApiKey: '替换 API Key',
    ...
    settings: {
      ...
      defaults: {
        ...
        dailyCheckinEnabledHint: '开启后，普通用户可在每日签到页每天转盘抽奖一次',
        ...
        dailyCheckinPrizePool: '转盘奖池',
        dailyCheckinPrizePoolHint: '配置转盘可抽中的奖励，启用奖项概率合计必须等于 100%。',
        dailyCheckinProbabilityTotal: '合计 {total}%',
        dailyCheckinAddPrize: '添加奖项',
        dailyCheckinPrizeEmpty: '未配置奖项。开启每日签到前至少需要一个启用奖项。',
        dailyCheckinPrizeName: '奖项名称',
        dailyCheckinPrizeType: '奖励类型',
        dailyCheckinPrizeTypeBalance: '余额',
        dailyCheckinPrizeTypeConcurrency: '并发额度',
        dailyCheckinPrizeTypeSubscription: '订阅套餐',
        dailyCheckinPrizeTypeNone: '谢谢参与',
        dailyCheckinProbabilityBps: '概率基点',
        dailyCheckinBalanceMode: '余额模式',
        dailyCheckinConcurrency: '增加并发数',
        dailyCheckinNoneHint: '该奖项不会发放额度，用于承接衰减后的空奖概率。',
        dailyCheckinDecay: '未付费衰减',
        dailyCheckinDecayHint: '未产生真实付费订单的用户，试用期后会按规则降低非空奖项的有效概率，差额转为谢谢参与。',
        dailyCheckinUnpaidFullDays: '完整概率天数',
        dailyCheckinDecayAfterDays: '注册满天数',
        dailyCheckinDecayFactor: '奖励概率系数',
        dailyCheckinLinuxDoExempt: 'LinuxDo 用户豁免',
        dailyCheckinLinuxDoExemptHint: '开启后，来自 LinuxDo 注册或绑定 LinuxDo 身份的用户不受未付费衰减影响。',
        dailyCheckinPrizesRequired: '开启每日签到时至少需要一个启用奖项',
        dailyCheckinProbabilityInvalid: '启用奖项的概率合计必须等于 100%',
        dailyCheckinPrizeNameRequired: '启用奖项名称不能为空',
        dailyCheckinBalanceFixedInvalid: '余额固定奖励必须大于 0',
        dailyCheckinBalanceRangeInvalid: '余额随机奖励最大值必须大于等于最小值',
        dailyCheckinBalanceRangeMaxInvalid: '余额随机奖励最大值必须大于 0',
        dailyCheckinConcurrencyInvalid: '并发奖励必须大于 0',
        dailyCheckinSubscriptionInvalid: '订阅奖励必须选择分组并设置有效期',
      ...
      semanticErrorDetection: {
        title: '2xx 语义错误识别',
        description: '识别上游 2xx 响应中的错误语义，并控制命中后的客户端返回内容和账号调度状态',
        enabled: '启用 2xx 语义错误识别',
        enabledHint: '当上游返回 2xx 但短响应内容命中错误语义时，返回自定义错误内容；开启“上游报错后停止调度”的账号会自动停调度。',
        maxChars: '匹配字符阈值',
        maxCharsHint: '仅检查不超过该字符数的响应，范围 128 到 65536，默认 4096。',
        rules: '语义匹配规则',
        rulesHint: '按优先级从小到大匹配；平台全不选表示适用于所有平台。',
        rulesEmpty: '暂无规则，添加后才会进行语义匹配。',
        ruleName: '规则名称',
        priority: '优先级',
        pattern: '匹配内容或正则表达式',
        customMessage: '命中后返回给客户端的自定义内容',
        platformsEmpty: '全不选 = 全平台',
        regexInvalid: '语义错误规则「{name}」的正则表达式无效。',
        ruleRequired: '语义错误规则的名称、匹配内容和自定义返回内容不能为空。',
      },
      webSearchEmulation: {
        title: 'Web Search 模拟',
      ...
      payment: {
        ...
        bonusRules: '充值返利规则',
        bonusRulesHint: 'JSON 数组。min_amount 含边界，max_amount 不含边界；max_amount 为 null 表示无上限。',
  ...
  payment: {
    ...
    admin: {
      ...
      retrySuccess: '重试成功',
      deleteOrder: '删除订单',
      deleteOrderConfirm: '确定要删除订单 #{id} 吗？当前状态：{status}。此操作不可撤销，删除后订单将不再出现在列表中。',
