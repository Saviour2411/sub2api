export default {
  ...
  affiliate: {
    ...
    tips: {
      ...
      line2: 'When invitees recharge balance, you receive {rate} of the credited balance amount as rebate quota.',
  ...
  dailyCheckin: {
    ...
    description: 'Daily check-in wheel',
    ...
    paid: 'Paid users use the normal prize pool',
    linuxdoExempt: 'LinuxDo exclusive pool is active',
    doneHint: 'Daily check-in is already completed',
    result: 'Result',
    prizePool: 'Prize List',
    recent: 'Recent Records',
    empty: 'No check-in records yet',
    days: ' days',
    success: 'Checked in: {reward}',
  ...
  admin: {
    ...
    channels: {
      ...
      form: {
        ...
        defaultPricingHint: 'Use these token prices when model-specific and global pricing both miss.',
        defaultPricingRequired: 'Default pricing requires at least one price',
        selectedCount: '{count} selected',
        searchGroups: 'Search groups...',
    ...
    riskControl: {
      ...
      localAuditEnabled: 'Save successful conversations for manual audit',
      localAuditEnabledHint: 'When enabled, only successfully forwarded conversations are stored as local files. Failed requests are not stored; sensitive fields are redacted.',
      localAuditMaxStorage: 'Local retention cap',
      localAuditMaxCaptureConcurrency: 'Audit capture concurrency cap',
      localAuditMaxCaptureConcurrencyHint: 'When the cap is reached, user data saving is temporarily skipped while requests continue normally. Set 0 for unlimited; not recommended in production.',
      localAuditScenePolicy: 'Save scene',
      localAuditSceneAll: 'All successful conversations',
      localAuditSceneAllDesc: 'Save every successfully forwarded conversation, useful when expanding manual review temporarily.',
      localAuditSceneProgrammingOnly: 'Programming only',
      localAuditSceneProgrammingOnlyDesc: 'Save only when programming signals such as client, tool calls, or file reads/writes are detected.',
      localAuditExcludeImage: 'Exclude image generation',
      localAuditExcludeImageHint: 'Skip Images generation/edit requests to reduce unnecessary audit files.',
      localAuditClientPatterns: 'Programming client patterns',
      localAuditToolPatterns: 'Programming tool patterns',
      localAuditPatternsHint: 'One keyword per line, case-insensitive; matched against User-Agent or Originator.',
      localAuditToolPatternsHint: 'One keyword per line, case-insensitive; matched against tool names in requests or responses.',
      localAuditStoragePath: 'Storage path',
      localAuditStorageStats: '{records} records, {bytes} used, {max} GB cap. Oldest records are removed automatically after the cap is exceeded.',
      localAuditStabilityTitle: 'Stability fallback',
      localAuditStabilityHint: 'Current capture active: {active}; overload skips: {skipped}. Estimate memory as concurrency cap {concurrency} x per-request capture {limit} x 4-5, plus baseline service memory and Go GC headroom. Start with 128 on the current server.',
      localAuditRecords: 'Manual Audit Records',
      localAuditRecordsHint: 'Locally stored successful conversation traces for admin review.',
      localAuditLoadFailed: 'Failed to load manual audit records',
      localAuditDetailFailed: 'Failed to load manual audit detail',
      localAuditDownloadFailed: 'Failed to download manual audit record',
      localAuditDeleteFailed: 'Failed to delete manual audit record',
      localAuditDeleted: 'Manual audit record deleted',
      localAuditDeleteConfirm: 'Delete this manual audit record? This cannot be undone.',
      localAuditEmpty: 'No manual audit records',
      localAuditCounts: 'Messages {messages} / tools {tools} / calls {calls} / results {results}',
      localAuditDetailTitle: 'Manual Audit Detail',
      localAuditDetailContent: 'Conversation JSON',
      download: 'Download',
      ...
      localAuditFilters: {
        model: 'Search model',
        search: 'Search user/key/session',
      },
      localAuditTable: {
        counts: 'Trace Counts',
        size: 'File Size',
    ...
    settings: {
      ...
      defaults: {
        ...
        dailyCheckinEnabledHint: 'Allow regular users to spin the daily check-in wheel once per day',
        ...
        dailyCheckinPrizePool: 'Wheel Prize Pool',
        dailyCheckinPrizePoolHint: 'Configure wheel rewards. Enabled prize probabilities must add up to 100%.',
        dailyCheckinProbabilityTotal: 'Total {total}%',
        dailyCheckinAddPrize: 'Add Prize',
        dailyCheckinPrizeEmpty: 'No prizes configured. Add at least one enabled prize before enabling daily check-in.',
        dailyCheckinPrizeName: 'Prize Name',
        dailyCheckinPrizeType: 'Reward Type',
        ...
        dailyCheckinPrizeTypeNone: 'No Reward',
        dailyCheckinProbabilityBps: 'Probability Bps',
        dailyCheckinBalanceMode: 'Balance Mode',
        dailyCheckinConcurrency: 'Added Concurrency',
        dailyCheckinNoneHint: 'This prize does not grant quota and receives the empty probability shifted by decay.',
        dailyCheckinDecay: 'Unpaid Decay',
        dailyCheckinDecayHint: 'Users without completed paid orders get lower effective non-empty prize probability after the trial window. The difference becomes no reward.',
        dailyCheckinUnpaidFullDays: 'Full Probability Days',
        dailyCheckinDecayAfterDays: 'Registered Days',
        dailyCheckinDecayFactor: 'Reward Probability Factor',
        dailyCheckinLinuxDoExempt: 'LinuxDo Exemption',
        dailyCheckinLinuxDoExemptHint: 'When enabled, users registered from LinuxDo or bound to a LinuxDo identity are exempt from unpaid decay.',
        dailyCheckinPrizesRequired: 'At least one enabled prize is required when daily check-in is enabled',
        dailyCheckinProbabilityInvalid: 'Enabled prize probabilities must add up to 100%',
        dailyCheckinPrizeNameRequired: 'Enabled prize name cannot be empty',
        dailyCheckinBalanceFixedInvalid: 'Fixed balance reward must be greater than 0',
        dailyCheckinBalanceRangeInvalid: 'Maximum balance reward must be greater than or equal to minimum reward',
        dailyCheckinBalanceRangeMaxInvalid: 'Maximum balance reward must be greater than 0',
        dailyCheckinConcurrencyInvalid: 'Concurrency reward must be greater than 0',
        dailyCheckinSubscriptionInvalid: 'Subscription reward requires a group and validity days',
      ...
      semanticErrorDetection: {
        title: '2xx Semantic Error Detection',
        description: 'Detect semantic errors in upstream 2xx responses and control the client response plus account scheduling state',
        enabled: 'Enable 2xx Semantic Error Detection',
        enabledHint: 'When a short 2xx upstream response matches an error semantic rule, return your custom error message; accounts using the upstream-error stop-scheduling strategy are disabled automatically.',
        maxChars: 'Match Character Limit',
        maxCharsHint: 'Only responses at or below this character count are checked. Range: 128 to 65536. Default: 4096.',
        rules: 'Semantic Rules',
        rulesHint: 'Rules match by ascending priority; selecting no platform means all platforms.',
        rulesEmpty: 'No rules yet. Semantic matching starts after a rule is added.',
        ruleName: 'Rule name',
        priority: 'Priority',
        pattern: 'Text or regular expression to match',
        customMessage: 'Custom content returned to the client on match',
        platformsEmpty: 'None selected = all platforms',
        regexInvalid: 'The regular expression for semantic error rule "{name}" is invalid.',
        ruleRequired: 'Semantic error rule name, pattern, and custom response content are required.',
      },
      webSearchEmulation: {
      ...
      payment: {
        ...
        bonusRules: 'Recharge Bonus Rules',
        bonusRulesHint: 'JSON array. min_amount is inclusive, max_amount is exclusive; null max means no upper limit.',
  ...
  payment: {
    ...
    admin: {
      ...
      deleteOrderConfirm: 'Delete order #{id}? Current status: {status}. This cannot be undone and the order will no longer appear in the list.',
