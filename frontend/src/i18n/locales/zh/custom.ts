export default {
  userCustomization: {
    title: '用户定制', paymentMethods: '全局充值方式', methodKey: '名称，例如币安 ID', methodValue: '收款信息',
    noPaymentMethods: '暂未配置充值方式', moveUp: '上移', moveDown: '下移', search: '搜索用户名或用户 ID',
    user: '用户', queryLink: '查询链接', autoCredit: '自动授信', noUsers: '暂无用户', totalUsers: '共 {count} 位用户',
    linkActive: '已启用', linkDisabled: '已停用', noLink: '未生成', copyLink: '复制查询链接', generateLink: '生成链接',
    disableLink: '停用', enableLink: '启用', resetLink: '重置链接', configure: '授信配置',
    threshold: '余额触发阈值', amount: '单次授信金额', used: '本轮已使用', ready: '待触发', off: '已关闭',
    restoreCredit: '恢复自动授信', temporaryCredit: '临时授信',
    restoreConfirm: '为 {user}（ID {id}）恢复一次自动授信？余额低于 {threshold} 时将增加 {amount}；若当前已低于阈值，下次后台检查即可入账。',
    resetLinkConfirm: '重置 {user}（ID {id}）的查询链接？旧链接将立即失效，授信状态不变。',
    loadFailed: '加载失败，请重试', invalidMethods: '请填写完整的充值方式名称和收款信息',
    saved: '已保存', saveFailed: '保存失败，请检查配置后重试', operationFailed: '操作未完成，已刷新用户状态，请检查后重试',
    copied: '查询链接已复制', copyFailed: '复制失败，请检查浏览器剪贴板权限'
  },
  balanceQuery: {
    title: '余额查询', balance: '当前余额', refresh: '刷新余额', paymentMethods: '充值方式', history: '充值记录',
    empty: '暂无充值记录', time: '时间', amount: '到账金额', note: '备注',
    unavailable: '查询链接不可用', rateLimited: '查询过于频繁，请稍后重试', serviceUnavailable: '查询暂时不可用，请稍后重试',
    retryAfter: '{seconds} 秒后可重试', copy: '复制收款信息', copied: '已复制', copyFailed: '复制失败，请检查浏览器剪贴板权限'
  },
  common: {
    previous: '上一页',
    retry: '重试',
    apply: '应用',
    clear: '清空',
    creating: '创建中...',
    required: '必填',
    sending: '发送中...',
    tryAgain: '重试',
  },
  nav: {
    customFeatures: '二开功能',
  },
  dailyCheckin: {
    title: '每日签到',
    description: '每日签到抽取奖励',
    kicker: '每日福利',
    balance: '当前余额',
    factor: '专属奖池',
    linuxdoPool: 'LinuxDo 专属奖池',
    result: '中奖结果',
    prizePool: '奖品列表',
    empty: '暂无记录',
    recent: '最近记录',
    loading: '加载中',
    spinning: '抽奖中',
    done: '今日已签',
    spin: '开始抽奖',
    disabled: '签到未开启',
    doneHint: '今天已经签到过了',
    linuxdoExempt: 'LinuxDo 用户享受衰减豁免',
    ready: '今日可签到',
    loadFailed: '加载签到信息失败',
    success: '签到成功：{reward}',
    failed: '签到失败',
    days: '天',
    types: {
      balance: '余额',
      concurrency: '并发',
      subscription: '订阅',
      none: '谢谢参与',
    },
  },
  payment: {
    bonusAmount: '返利金额',
    rechargeBonusRulePreview: '当前金额命中返利规则，返利比例为 {rate}%。',
  },
  redeem: {
    dailyCheckinReward: '签到奖励',
    balanceAddedDailyCheckin: '每日签到奖励',
  },
  admin: {
    accounts: {
      fromModel: '源模型',
      toModel: '目标模型',
      messages: {
        accountCreated: '账号创建成功',
      },
      oauth: {
        openai: {
          accessTokenAuth: 'Access Token 授权',
          mobileRefreshTokenAuth: '移动端 Refresh Token 授权',
        },
      },
      openai: {
        imageSizeTiers: '支持生图规格',
        imageSizeTiersDesc: '选择该 OpenAI 账号支持的生图规格；未配置时沿用默认路由逻辑。',
        codexCLIEmulation: 'Codex CLI 兼容模式',
        codexCLIEmulationDesc: '为 OpenAI API Key 账号启用 Codex CLI 兼容请求处理。',
      },
      failureSchedulingStrategy: {
        title: '失败调度策略',
        default: '默认策略',
        disableUntilTestPass: '失败后暂停直到测试通过',
        hint: '控制账号在指定失败状态码后是否继续参与调度。',
      },
      status: {
        lastScheduledTestFailure: '最近定时测试失败',
        unschedulableReasonTitle: '不可调度原因',
        unschedulableReason: '原因',
        unschedulableFirstTokenTimeout: '连续首 Token 超时',
        unschedulableUpstreamError: '连续上游错误',
        unschedulableStatusCode: '状态码',
        unschedulableConsecutiveCount: '连续次数',
        unschedulableThreshold: '停调度阈值',
        unschedulableModel: '模型',
        unschedulableTimeoutSeconds: '首 Token 超时秒数',
        unschedulableOccurredAt: '发生时间',
      },
    },
    channelMonitor: {
      advanced: {
        streamEnabled: '启用流式请求',
        streamEnabledHint: '使用流式响应检测渠道可用性。',
      },
    },
    channels: {
      noGroupsSelected: '请至少选择一个分组',
      emptyModelsInPricing: '请先添加模型定价',
      form: {
        defaultPricing: '默认定价',
        defaultPricingHint: '当模型未配置专属价格时使用该默认价格。',
        defaultPricingRequired: '请填写默认定价',
      },
    },
    ops: {
      runtime: {
        metricThresholds: '指标阈值',
        metricThresholdsHint: '配置运维健康评分与告警阈值。',
        slaMinPercent: 'SLA 最低百分比',
        slaMinPercentHint: '低于该值时标记为异常。',
        ttftP99MaxMs: 'TTFT P99 最大值（ms）',
        ttftP99MaxMsHint: '首 Token P99 延迟超过该值时触发告警。',
        requestErrorRateMaxPercent: '请求错误率上限 (%)',
        requestErrorRateMaxPercentHint: '请求错误率超过该值时触发告警。',
        upstreamErrorRateMaxPercent: '上游错误率上限 (%)',
        upstreamErrorRateMaxPercentHint: '上游错误率超过该值时触发告警。',
      },
    },
    redeem: {
      viewUsages: '查看使用记录',
      usageRecords: '使用记录',
      noUsages: '暂无使用记录',
      failedToLoadUsages: '加载使用记录失败',
      usageUser: '使用用户',
      maxUses: '最大使用次数',
      columns: {
        usage: '使用情况',
      },
    },
    riskControl: {
      download: '下载',
      localAuditEnabled: '启用本地审计捕获',
      localAuditEnabledHint: '捕获命中的请求内容到本地审计目录，用于人工复核。',
      localAuditStoragePath: '本地审计存储路径',
      localAuditMaxStorage: '最大存储空间',
      localAuditMaxCaptureConcurrency: '最大捕获并发',
      localAuditMaxCaptureConcurrencyHint: '限制本地审计写入的并发数量。',
      localAuditExcludeImage: '排除生图请求',
      localAuditExcludeImageHint: '开启后不捕获图片生成请求内容。',
      localAuditScenePolicy: '捕获场景',
      localAuditSceneAll: '全部请求',
      localAuditSceneAllDesc: '捕获所有命中本地审计规则的请求。',
      localAuditSceneProgrammingOnly: '仅编程相关',
      localAuditSceneProgrammingOnlyDesc: '仅捕获疑似编程和工具调用相关请求。',
      localAuditClientPatterns: '客户端匹配规则',
      localAuditToolPatterns: '工具匹配规则',
      localAuditToolPatternsHint: '按工具名称或工具描述匹配需要捕获的请求。',
      localAuditPatternsHint: '每行一个匹配规则，支持普通字符串或正则。',
      localAuditStorageStats: '存储统计',
      localAuditRecords: '本地审计记录',
      localAuditRecordsHint: '查看本地捕获的审计请求。',
      localAuditEmpty: '暂无本地审计记录',
      localAuditLoadFailed: '加载本地审计记录失败',
      localAuditDownloadFailed: '下载本地审计记录失败',
      localAuditDeleted: '本地审计记录已删除',
      localAuditDeleteFailed: '删除本地审计记录失败',
      localAuditDeleteConfirm: '确定删除这条本地审计记录吗？',
      localAuditDetailTitle: '本地审计详情',
      localAuditDetailContent: '审计内容',
      localAuditDetailFailed: '加载审计详情失败',
      localAuditCounts: '捕获 {count} 条',
      localAuditStabilityTitle: '稳定性说明',
      localAuditStabilityHint: '本地审计捕获队列独立于异步审核队列，队列积压时会优先保护主请求链路。',
      localAuditFilters: {
        search: '搜索内容',
        model: '模型',
      },
      localAuditTable: {
        size: '大小',
        counts: '命中次数',
      },
    },
    customFeatures: {
      title: '二开功能',
      description: '集中管理本项目扩展的上游监控、模型广场、网关与每日签到功能。',
      enabled: '启用',
      save: '保存配置',
      loadFailed: '加载二开功能配置失败',
      saveFailed: '保存二开功能配置失败',
      tabs: {
        upstream: '上游管理',
        canvas: '无限画布',
        modelMarketplace: '模型广场',
        gateway: '网关配置',
        dailyCheckin: '每日签到',
      },
      canvas: {
        title: '无限画布',
        description: '控制用户侧无限画布入口以及画布初始化和运行时凭据接口。关闭后已打开的画布也无法继续解析站内凭据。',
        saved: '无限画布配置已保存',
      },
      upstream: {
        title: '上游管理',
        description: '统一查看上游账号余额、分组倍率、实际消耗和接入后的用量历史。',
        add: '新增上游',
        addTitle: '新增上游站点',
        editTitle: '编辑上游站点',
        syncAll: '同步全部',
        sortOrder: '排序站点',
        sortOrderHint: '拖拽调整站点显示顺序，保存后默认列表和站点卡片都会使用该顺序。',
        sortOrderLoadFailed: '加载站点排序失败',
        sortOrderSaveFailed: '保存站点排序失败',
        sortOrderUpdated: '站点排序已更新',
        sync: '立即同步',
        showGroups: '展示可用分组',
        hideGroups: '收起可用分组',
        searchPlaceholder: '搜索名称、地址或账号',
        allPlatforms: '全部平台',
        enabledState: '启用状态',
        allStates: '全部状态',
        groupPlatform: '分组类型',
        allGroupPlatforms: '全部分组类型',
        sortBy: '站点排序',
        defaultSort: '默认顺序',
        sort: {
          balanceDesc: '余额：高到低',
          balanceAsc: '余额：低到高',
          todayTokensDesc: '今日 Token：高到低',
          todayTokensAsc: '今日 Token：低到高',
        },
        enabled: '已启用',
        disabled: '已禁用',
        enable: '启用',
        disable: '禁用',
        platform: '平台',
        openSite: '打开上游网站',
        details: '查看详情',
        edit: '编辑',
        delete: '删除',
        empty: '暂无上游站点',
        loadFailed: '加载上游站点失败',
        saveFailed: '保存上游站点失败',
        syncFailed: '上游同步排队失败',
        deleteFailed: '删除上游站点失败',
        detailFailed: '加载上游详情失败',
        groupsLoadFailed: '加载可用分组失败',
        multiplierHistoryFailed: '加载倍率历史失败',
        groupDisplayFailed: '更新分组展示状态失败',
        saved: '上游站点已保存',
        deleted: '上游站点已删除',
        groupDisplayed: '分组已添加到账号下方',
        groupHidden: '分组已从账号下方隐藏',
        bindings: {
          title: '绑定账号 - {name}',
          open: '配置该上游分组绑定的本地账号',
          button: '绑定账号',
          multiplier: '当前倍率',
          boundCount: '已绑定 {count} 个账号',
          globalPriorityWarning: '账号优先级是全局配置。绑定账号属于多个本地分组时，自动同步的优先级会同时影响这些分组；如需长期手工控制，请先解除绑定。',
          frozenHint: '该上游分组当前不可用或倍率为空，不能新增账号；已有绑定会继续使用最后一次有效倍率参与优先级排序，从未取得有效倍率的账号保留当前优先级。',
          currentBindings: '当前绑定',
          account: '账号',
          localGroup: '本地分组',
          status: '状态',
          priority: '全局优先级',
          remove: '解除绑定',
          noBindings: '暂无已绑定账号',
          searchAccount: '搜索分组账号',
          searchPlaceholder: '输入账号名称',
          search: '搜索',
          noActiveGroups: '暂无可选择的活跃分组',
          groupsLoadFailed: '加载本地分组失败',
          accountsLoadFailed: '加载分组账号失败',
          noAccounts: '该分组下没有匹配的账号',
          selected: '已选择',
          addAccount: '添加',
          saved: '账号绑定已保存',
          conflict: '所选账号已绑定到其他上游分组，请刷新后重新选择',
          dataChanged: '绑定或倍率状态已发生变化，请重新打开后再编辑',
          saveFailed: '保存账号绑定失败',
          unbindBeforeHide: '该上游分组仍有绑定账号，请先解除全部绑定',
          deleteWarning: '删除站点将同时解除 {count} 个账号绑定；账号当前优先级会保留。',
        },
        pagination: '第 {page}/{pages} 页，共 {total} 个站点',
        previous: '上一页',
        next: '下一页',
        name: '站点名称',
        baseUrl: '上游地址',
        authMode: '认证方式',
        passwordAuth: '账号密码',
        tokenAuth: '访问令牌',
        probingCapabilities: '正在检测站点认证能力...',
        capabilityProbeFailed: '无法检测站点认证能力，可继续尝试保存',
        turnstileDetectedTitle: '目标站点已开启 Cloudflare Turnstile',
        turnstileDetectedDescription: '账号密码自动登录会被人机验证拦截。请先在目标站完成登录，再使用访问令牌和刷新令牌接入。',
        turnstileRequiresToken: '目标站点开启了 Cloudflare Turnstile，请改用访问令牌认证',
        openLoginPage: '打开登录页',
        account: '账号',
        password: '密码',
        accessToken: '访问令牌',
        newAPIAccessTokenHint: '填写 New API 个人中心生成的个人访问令牌；该模式不使用账号密码、Cookie 或刷新令牌，也无需填写会话 User-Agent。',
        refreshToken: '刷新令牌',
        sessionUserAgent: '会话 User-Agent',
        sessionUserAgentHint: '目标站点绑定会话指纹时，验证、刷新和同步请求会始终使用此值。导入登录响应时会自动记录当前浏览器 User-Agent。',
        keepSessionUserAgent: '留空保留现有会话 User-Agent',
        loginResponse: '登录接口响应',
        loginResponsePlaceholder: '粘贴 POST /api/v1/auth/login 返回的完整 JSON',
        loginResponseHint: '系统只会提取 access_token 和 refresh_token，导入成功后立即清空此输入框。',
        importTokens: '导入令牌',
        loginResponseInvalid: '登录响应不是有效的 JSON',
        loginResponseMissingTokens: '登录响应中未找到访问令牌或刷新令牌',
        tokensImported: '登录令牌已导入',
        keepCredential: '留空保留现有凭证',
        tokenRequired: '请至少填写访问令牌或刷新令牌',
        newAPITokenRequired: '请填写 New API 个人访问令牌',
        cancel: '取消',
        save: '保存',
        validating: '验证连接中...',
        deleteTitle: '删除上游站点',
        deleteMessage: '确定删除“{name}”及其全部分组和历史记录吗？',
        noGroups: '暂无可用分组',
        noDisplayedGroups: '暂无已添加展示的分组',
        available: '可用',
        unavailable: '暂不可用',
        addGroupDisplay: '添加展示',
        hideGroup: '隐藏',
        viewMultiplierTrend: '查看倍率趋势',
        viewMultiplierTrendFor: '查看{name}的倍率趋势',
        lastSeenAt: '末次同步：{time}',
        noHistory: '所选日期范围暂无历史数据',
        noMultiplierHistory: '所选日期范围暂无倍率记录',
        multiplierGroup: '分组',
        columns: {
          site: '站点',
          status: '状态',
          balance: '余额',
          today: '今日 Token / 实际消耗',
          total: '累计 Token / 实际消耗',
          lastSync: '最后同步',
          actions: '操作',
        },
        status: {
          pending: '排队中',
          syncing: '同步中',
          healthy: '正常',
          error: '异常',
        },
        detailTabs: {
          groups: '可用分组',
          usage: '站点用量趋势',
          multiplier: '倍率趋势',
        },
        groupColumns: {
          name: '分组',
          status: '可用状态',
          multiplier: '生效倍率',
          tokens: '今日 Token',
          cost: '今日实际消耗',
        },
        chart: {
          tokens: 'Token',
          cost: '实际消耗（USD）',
          balance: '余额（USD）',
          multiplier: '倍率',
        },
      },
      modelMarketplace: {
        title: '模型广场',
        description: '配置公开页面展示的模型分组和顶部说明。',
        intro: '顶部描述文字',
        introPlaceholder: '例如：这里展示当前可用模型和兼容接口。',
        introHint: '按纯文本渲染，支持换行，不执行 HTML。',
        groups: '展示分组',
        selectedCount: '已选择 {count} 个分组',
        groupsHint: '不选择任何分组时，只展示活跃的公开标准分组；选择后可显式展示活跃的专属或订阅分组。',
        noGroups: '暂无活跃分组',
        exclusive: '专属',
        subscription: '订阅',
        saved: '模型广场配置已保存',
      },
      gateway: {
        title: '网关配置',
        description: '配置账号池默认值、附加换号状态码、自动测活退避、流式首 Token 超时和 Image 分组成功率。',
        minutes: '分钟',
        saved: '网关配置已保存',
        streamSafeRetry: {
          title: 'Claude 透传流安全重试',
          description: '仅适用于 Claude API Key 透传流。默认关闭；尚未交付内容时，有限重试空流、EOF、流空闲及首有效内容超时。',
          maxRetries: '额外重试次数（0–5）',
          budget: '总等待预算（秒，1–3600）',
          firstContentTimeout: '首有效内容超时（秒，0–3600）',
          hint: '首有效内容超时默认180秒，0关闭；每次成功流响应头到达后独立计时，心跳和纯空白不续期，文本、思考和工具调用均算有效内容。总预算从首次成功流响应头开始跨尝试累计，重试不重置；不是整个生成时长。先原账号、再换号，无替代账号时允许原号使用剩余次数，仍受账号资格限制。已提交内容禁止重放。0次重试表示不重发，前导保护和超时仍生效。',
          validation: '额外重试次数须为0–5的整数，总等待预算须为1–3600秒的整数，首有效内容超时须为0–3600秒的整数（0关闭）。'
        },
        anthropicClaudeCodeMimicry: {
          title: 'Anthropic Claude Code 上游模拟',
          description: '开启后，所有 Anthropic 类型分组下未开启透传模式的 API Key 账号会模拟 Claude Code 客户端请求。OAuth 账号仍保持自动模拟，真实 Claude Code 请求不会重写。',
          enabled: '启用模拟',
        },
        anthropicSamplingParameterFilter: {
          title: 'Anthropic 采样参数过滤',
          description: '命中最终上游模型后，从 REST Messages 请求体中删除 temperature、top_k 和 top_p。支持精确模型 ID 与末尾通配符。',
          enabled: '启用过滤',
        },
        customRateRechargeBonus: {
          title: '专属倍率用户充值不返利',
          description: '开启后，只要用户在任意分组配置了专属倍率，余额充值就按原始充值金额入账，不应用阶梯返利或旧版充值倍率。',
          enabled: '启用限制',
        },
        poolDefaults: {
          title: '账号池默认值',
          description: '仅应用于后续新建的 API Key 和 Bedrock 账号，不修改存量账号。',
          retryCount: '池内重试次数',
          retryCountHint: '允许 0 到 10 次；新账号默认开启池模式。',
          retryStatusCodes: '池内重试状态码',
          retryStatusCodesHint: '使用逗号或空格分隔，可留空；保存时会去重并按升序排列。',
        },
        additionalFailoverStatusCodes: {
          title: '附加换号状态码',
          description: '开启后，命中下列上游 HTTP 状态码也会进入现有账号换号流程。该列表只补充内置规则，不会关闭内置换号条件，也不会覆盖请求级语义终止保护。',
          enabled: '启用附加换号',
          statusCodes: '附加状态码',
          statusCodesHint: '使用逗号或空格分隔，范围为 100 到 599；保存时会去重并按升序排列。默认预填 451。',
        },
        probeBackoff: {
          title: '自动测活退避',
          description: '连续失败时依次使用下列间隔，超过最后一档后持续使用最后一档。',
          attempt: '第 {count} 次失败后',
          add: '添加退避档位',
          remove: '删除该退避档位',
        },
        firstTokenTimeout: {
          scope: '生效范围',
          all: '全部分组',
          selected_groups: '指定分组',
          groups: '分组',
          unavailableGroup: '分组 #{id}（已不可用）',
          accountWideWarning: '达到连续超时阈值后，账号将在所有关联分组停调度；首 Token 达速且测活成功后恢复。',
          title: '流式首 Token 超时',
          description: '上游在时限内未返回有效首 Token 时停止等待、跳过当前账号池内重试并切换账号。',
          seconds: '最长等待秒数',
          hint: '允许 0 到 600 秒；设为 0 表示关闭。',
          consecutiveThreshold: '连续超时停调度次数',
          consecutiveThresholdHint: '允许 1 到 100 次；达到阈值后账号将不可调度，测活通过后恢复。',
        },
        upstreamError: {
          title: '连续上游错误停调度',
          description: '账号连续命中指定上游状态码并达到阈值后，将停止参与调度并启动自动测活。',
          consecutiveThreshold: '连续错误停调度次数',
          consecutiveThresholdHint: '允许 1 到 100 次；一次成功请求会重置该账号的连续错误计数。',
          statusCodes: '计入连续错误的状态码',
          statusCodesHint: '使用逗号或空格分隔，可留空；保存时会去重并按升序排列。',
        },
        imageSuccessRate: {
          title: 'Image 分组成功率',
          description: '控制渠道状态页是否展示 Image 分组成功率；关闭展示后仍会继续累计。',
          visible: '渠道状态页展示',
          reset: '清除成功率统计',
          resetTitle: '清除 Image 分组成功率统计',
          resetMessage: '清除后将从当前时间重新统计，历史请求不会回算。此操作不可撤销。',
          resetConfirm: '确认清除',
          resetting: '正在清除...',
          resetSuccess: 'Image 分组成功率统计已清除',
          resetFailed: '清除 Image 分组成功率统计失败',
        },
        validation: {
          retryCount: '池内重试次数必须是 0 到 10 之间的整数',
          retryStatusCodes: '重试状态码必须是 100 到 599 之间的整数，并使用逗号或空格分隔',
          additionalFailoverStatusCodes: '附加换号状态码必须是 100 到 599 之间的整数，并使用逗号或空格分隔',
          additionalFailoverStatusCodesRequired: '启用附加换号状态码时必须至少填写一个状态码',
          probeBackoffRange: '自动测活退避必须包含 1 到 10 个、范围为 1 到 1440 的整数',
          probeBackoffOrder: '自动测活退避必须按非递减顺序排列',
          firstTokenTimeout: '首 Token 超时必须是 0 到 600 之间的整数',
          firstTokenGroups: '请至少选择一个生效分组',
          firstTokenConsecutiveThreshold: '首 Token 连续超时停调度次数必须是 1 到 100 之间的整数',
          upstreamErrorConsecutiveThreshold: '连续上游错误停调度次数必须是 1 到 100 之间的整数',
          upstreamErrorStatusCodes: '连续上游错误状态码必须是 100 到 599 之间的整数，并使用逗号或空格分隔',
          samplingParameterFilterModelsRequired: '启用 Anthropic 采样参数过滤时必须至少选择一个模型',
          samplingParameterFilterModelPattern: '模型模式仅允许精确 ID 或末尾通配符，且不能使用单独的 *',
        },
      },
      dailyCheckin: {
        title: '每日签到',
        description: '配置签到奖池以及从未付费用户的概率衰减规则。',
        prizePool: '签到奖池',
        prizePoolHint: '启用奖项的概率合计必须等于 100%，衰减后的差额会转入“谢谢参与”。',
        probabilityTotal: '合计 {total}%',
        addPrize: '添加奖项',
        noPrizes: '暂无奖项，请先添加奖项。',
        newPrize: '新奖项',
        status: '状态',
        prizeName: '奖项名称',
        prizeType: '奖励类型',
        probabilityBps: '概率基点',
        balanceMode: '余额模式',
        fixed: '固定额度',
        range: '随机范围',
        amount: '奖励额度',
        minAmount: '最小额度',
        maxAmount: '最大额度',
        concurrency: '增加并发数',
        subscriptionGroup: '订阅分组',
        noSubscriptionGroups: '暂无可用订阅分组',
        validityDays: '有效期（天）',
        noneHint: '该奖项不发放额度，用于承接空奖概率。',
        decayTitle: '未付费概率衰减',
        decayHint: '仅针对从未产生真实付费订单的用户，按账号注册天数降低非空奖项概率；付费一次后不再衰减。',
        fullDays: '完整概率天数',
        afterDays: '注册满天数',
        factorBps: '奖励概率系数',
        linuxdoExempt: 'LinuxDo 用户豁免',
        linuxdoExemptHint: '开启后，来自 LinuxDo 注册或已绑定 LinuxDo 身份的用户不受未付费衰减影响。',
        saved: '每日签到配置已保存',
        types: {
          balance: '余额',
          concurrency: '并发额度',
          subscription: '订阅套餐',
          none: '谢谢参与',
        },
        validation: {
          prizesRequired: '启用每日签到时至少需要一个启用奖项',
          probabilityTotal: '启用奖项的概率合计必须等于 100%',
          nameRequired: '启用奖项名称不能为空',
          probabilityRange: '奖项概率必须在 0 到 10000 基点之间',
          balanceAmount: '余额固定奖励必须大于 0',
          balanceRange: '余额随机奖励范围无效',
          concurrency: '并发奖励必须大于 0',
          subscription: '订阅奖励必须选择分组并设置有效期',
          decayRange: '衰减天数或概率系数超出允许范围',
        },
      },
    },
    users: {
      passwordCopied: '密码已复制',
    },
  },
}
