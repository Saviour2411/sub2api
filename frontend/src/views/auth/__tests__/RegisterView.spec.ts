...
describe('RegisterView', () => {
  ...
  it('does not flash the promo-code field before disabled settings finish loading', async () => {
    ...
    expect(wrapper.get('#confirmPassword').attributes('disabled')).toBeDefined()
    expect(wrapper.get('#confirmPassword').element.parentElement?.querySelector('button')?.disabled).toBe(true)

    resolveSettings(publicSettings)
    await flushPromises()

    expect(wrapper.find('#promo_code').exists()).toBe(false)
    expect(wrapper.get('#confirmPassword').attributes('disabled')).toBeUndefined()
    expect(wrapper.get('#confirmPassword').element.parentElement?.querySelector('button')?.disabled).toBe(false)
  })
