...
describe('EmailVerifyView', () => {
  ...
  it('submits pending auth account creation when session storage has no pending metadata but auth store does', async () => {
    ...
    expect(apiClientPostMock.mock.calls[0][1].aff_code).toBeUndefined()
  ...
  it('submits the affiliate code when register data explicitly belongs to the pending oauth flow', async () => {
    authStoreState.pendingAuthSession = {
      token: 'pending-token-affiliate',
      token_field: 'pending_auth_token',
      provider: 'wechat',
      redirect: '/profile',
    }
    sessionStorage.setItem(
      ...
      JSON.stringify({
        ...
        password: 'secret-123',
        aff_code: 'AFF123',
        pending_provider: 'wechat',
        pending_auth_token: 'pending-token-affiliate',
        pending_auth_token_field: 'pending_auth_token',
      })
    )
    apiClientPostMock.mockResolvedValue({
      data: {
