...
const exportToExcel = async () => {
  ...
  const exportParams = { ...buildUsageListParams(1, USAGE_EXPORT_PAGE_SIZE, false) }
  try {
    ...
    const count = await exportUsagePages<AdminUsageLog>({
      load: (cursor, signal) => adminUsageAPI.exportPage({ ...exportParams, cursor }, { signal }),
      signal: c.signal,
      onProgress: (count) => { exportProgress.current = count },
      consume: (items) => {
        const rows = items.map((log: AdminUsageLog) => [
    ...
    if(!c.signal.aborted) {
      ...
      saveAs(new Blob([XLSX.write(wb, { bookType: 'xlsx', type: 'array' })], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }), `usage_${exportParams.start_date}_to_${exportParams.end_date}.xlsx`)
      appStore.showSuccess(t('usage.exportSuccess'))
    }
  } catch (error) {
    if (!c.signal.aborted && !isExportCanceled(error)) {
      console.error('使用记录导出失败:', error)
