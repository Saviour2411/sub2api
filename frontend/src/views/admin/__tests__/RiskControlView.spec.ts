...
describe('admin RiskControlView', () => {
  ...
  it.each(['openai', 'typesafe'] as const)('切换并保存 %s 引擎时保留本地审计和 Cyber Policy 配置', async (engine) => {
    const shared = {
      cyber_policy_exclude_from_ban_count: true,
      local_audit_enabled: true,
      local_audit_max_storage_gb: 2.5,
      local_audit_scene_policy: 'programming_only' as const,
      local_audit_exclude_image_generation: true,
      local_audit_programming_client_patterns: ['codex_', 'cursor'],
      local_audit_programming_tool_patterns: ['apply_patch', 'read_file'],
      local_audit_max_capture_concurrency: 17,
    }
    getConfig.mockResolvedValue({ ...baseConfig(), ...shared })
    ...
    await flushPromises()
    await findButtonByText(wrapper, 'admin.riskControl.openSettings').trigger('click')
    const selector = wrapper.findComponent('[data-test="audit-engine-select"]')
    selector.vm.$emit('update:modelValue', 'typesafe')
    await flushPromises()
    selector.vm.$emit('update:modelValue', engine)
    await flushPromises()
    await findButtonByText(wrapper, 'admin.riskControl.saveConfig').trigger('click')
    await flushPromises()
    expect(updateConfig).toHaveBeenCalledWith(expect.objectContaining({
      ...shared,
      engine,
      engine_configs: { openai: expect.any(Object), typesafe: expect.any(Object) },
    }))
    expect(showError).not.toHaveBeenCalled()
    wrapper.unmount()
  })
