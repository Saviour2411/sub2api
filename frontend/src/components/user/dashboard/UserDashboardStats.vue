<template>
  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    <!-- Balance -->
    <div v-if="!isSimple" :ref="(el) => setCardRef(el, 0)" class="card stat-deck-card p-4 reveal-up tilt-surface tilt-glow scan-host" data-reveal>
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0 flex-1">
          <div class="flex items-center justify-between gap-2">
            ...
            <PulseDot tone="success" :title="t('common.available')" />
          </div>
          <CountUp class="text-xl font-bold text-emerald-600 dark:text-emerald-400" :value="balance" :decimals="2" prefix="$" />
    ...
    <!-- API Keys -->
    <div :ref="(el) => setCardRef(el, 1)" class="card stat-deck-card p-4 reveal-up tilt-surface tilt-glow scan-host" data-reveal>
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0 flex-1">
          ...
          <CountUp class="text-xl font-bold text-gray-900 dark:text-white" :value="stats?.total_api_keys || 0" />
    ...
    <!-- Today Requests -->
    <div :ref="(el) => setCardRef(el, 2)" class="card stat-deck-card p-4 reveal-up tilt-surface tilt-glow scan-host" data-reveal>
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0 flex-1">
          ...
          <CountUp class="text-xl font-bold text-gray-900 dark:text-white" :value="stats?.today_requests || 0" />
    ...
    <!-- Today Cost -->
    <div :ref="(el) => setCardRef(el, 3)" class="card stat-deck-card p-4 reveal-up tilt-surface tilt-glow scan-host" data-reveal>
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0 flex-1">
          ...
          <p class="text-xl font-bold text-gray-900 dark:text-white">
            <CountUp class="text-purple-600 dark:text-purple-400" :value="stats?.today_actual_cost || 0" :decimals="4" prefix="$" :title="t('dashboard.actual')" />
  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    <!-- Today Tokens -->
    <div :ref="(el) => setCardRef(el, 4)" class="card stat-deck-card p-4 reveal-up tilt-surface tilt-glow scan-host" data-reveal>
      <div class="flex items-center gap-3">
    ...
    <!-- Total Tokens -->
    <div :ref="(el) => setCardRef(el, 5)" class="card stat-deck-card p-4 reveal-up tilt-surface tilt-glow scan-host" data-reveal>
      <div class="flex items-center gap-3">
    ...
    <!-- Performance (RPM/TPM) -->
    <div :ref="(el) => setCardRef(el, 6)" class="card stat-deck-card p-4 reveal-up tilt-surface tilt-glow scan-host" data-reveal>
      <div class="flex items-center gap-3">
    ...
    <!-- Avg Response Time -->
    <div :ref="(el) => setCardRef(el, 7)" class="card stat-deck-card p-4 reveal-up tilt-surface tilt-glow scan-host" data-reveal>
      <div class="flex items-center gap-3">
...
// Card refs + per-card tilt + staggered reveal
const cardRefs = ref<(HTMLElement | null)[]>([])
function setCardRef(el: Element | unknown, idx: number) {
  cardRefs.value[idx] = (el as HTMLElement | null) ?? null
}
...
function applyStaggeredReveal() {
  ...
  cards.forEach((c, i) => indexMap.set(c, i))
...
onMounted(() => {
  // Per-card tilt (each card binds independently)
  // Defer to next tick so refs are populated
...
const tiltDetachers: Array<() => void> = []
function attachTilt(el: HTMLElement) {
  ...
  function tick() {
    cx += (tx - cx) * ease
    cy += (ty - cy) * ease
    el.style.transform = `perspective(1100px) rotateX(${cy.toFixed(2)}deg) rotateY(${cx.toFixed(2)}deg) translateZ(${inside ? 4 : 0}px)`
  ...
  function enter() { inside = true; el.classList.add('is-tilting'); if (!raf) raf = requestAnimationFrame(tick) }
  function leave() { inside = false; tx = 0; ty = 0; if (!raf) raf = requestAnimationFrame(tick) }
...
onBeforeUnmount(() => {
  tiltDetachers.forEach((fn) => fn())
  tiltDetachers.length = 0
})
