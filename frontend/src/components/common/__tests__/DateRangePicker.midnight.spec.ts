...
async function reopenNextDay() {
  ...
  expect(page().findAll('input[type="date"]')[1].attributes('max')).toBe('2026-10-01')
  await w.get('.date-picker-trigger').trigger('click')
...
describe('date presets after midnight', () => {
  it.each([
    ...
    await page().findAll('.date-picker-preset').find(b => b.text() === label)!.trigger('click')
    await page().get('.date-picker-apply').trigger('click')
  ...
  it('updates the maximum selectable date when reopened', async () => {
    await reopenNextDay()
    expect(page().findAll('input[type="date"]')[1].attributes('max')).toBe('2026-10-02')
  })
})
