...
describe('CreateAccountModal OpenAI long-context billing', () => {
  ...
  it('同时保存请求标识头、图片回填开关和既有账号参数', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'OpenAI')
    await selectButtonByText(wrapper, 'API Key')
  ...
  it('includes the current concrete model mapping in preview credentials', async () => {
    ...
    expect(wrapper.getComponent(ModelWhitelistSelectorStub).props('previewSyncRequest')).toMatchObject({
