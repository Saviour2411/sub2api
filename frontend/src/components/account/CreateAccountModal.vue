...
const upstreamModelSyncPreviewRequest = computed<SyncUpstreamModelsPreviewRequest | null>(() => {
  ...
  return {
    ...
    type: 'apikey',
    credentials: {
      base_url: baseUrl || getDefaultApiKeyBaseUrl(form.platform),
      api_key: apiKey
    },
...
const handleSubmit = async () => {
  ...
  await doCreateAccount({
    ...form,
    group_ids: form.group_ids,
    extra: withUpstreamRequestIdHeader(applyFailureSchedulingStrategy(extra)),
...
const createAccountAndFinish = async (
  ...
  let finalExtra = withUpstreamRequestIdHeader(applyFailureSchedulingStrategy(extra))
    ...
    for (let i = 0; i < refreshTokens.length; i++) {
      try {
        ...
        const createPayload = withAntigravityConfirmFlag({
          ...
          type: 'oauth',
          credentials,
          extra: withUpstreamRequestIdHeader(applyFailureSchedulingStrategy({})),
          proxy_id: form.proxy_id,
          concurrency: form.concurrency,
          load_factor: form.load_factor ?? undefined,
          priority: form.priority,
